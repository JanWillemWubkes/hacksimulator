...
**Status:** MVP Development — ✅ LIVE on Netlify (laatste: Sessie 145)
...
**Rotation:** Keep last 6 full (140-145). Archive: docs/sessions/ (current.md, recent.md, archive-*.md)
Pre-Sessie 140 learnings (incl. Sessie 126 Brevo-migratie + 127 Typst PDF + 128 Gumroad factcheck/taalconsistentie + 129-133 monetization-stack + 134 Brevo DnD-herbouw + 135 DNS cleanup + 136 Postmaster re-check + 137 funnel-pulse + 138 OWASP hub-post + 139 unified marketing nav) → zie `docs/sessions/current.md`. Sessies 135/136/137/139 archived in bulk Sessie 145; 138 was 1-in-1-out Sessie 144. Volgende bulk-rotation trigger Sessie 150 (verwijdert 140-144 zodra 150 toegevoegd).
...
**Last updated:** 29 mei 2026 (Sessie 145 — Item #26 ✅ gesloten via Frame B (Lighthouse-attributie-bias) via commit `36cda01`. Verify-first-plan uit `/home/willem/.claude/plans/heisenberg-hier-pak-item-pure-cascade.md` uitgevoerd: 3-run Lighthouse@11 mobile met `--save-assets` + Python parse-script over raw `trace.json` + Playwright MCP cold/warm-meting via 5-iteratie cachebust dynamic-import. Multi-metric bewijs: raw trace box-utils.js totaal 1.3 ms X-phase dur (parse op worker-thread) vs Lighthouse-attributie 230 ms scripting = factor ~177x mismatch. Playwright coldCallMs 1.4 ms (Frame A drempel >20 ms gefalsifieerd) + warmCallMs 0.1 ms (hypothese (b) cache-failure gefalsifieerd). Hypotheses (a)/(b)/(c) uit Sessie 143-formulering allemaal gefalsifieerd door data — geen code-actie. Echte cost-driver uit `mainthread-work-breakdown`: Style/Layout 2172 ms. Item #28 spawned voor Layout/Style perf-audit. Bulk-rotation Sessie 135/136/137/139 → current.md.)
**Version:** 5.19 (Sessie 145 verify-first-plan-protocol introduceerde Frame B als legitieme ✅-closure-pad zonder code-wijziging — eerste sessie waar Heisenberg's verwachte impact ("+5-15 mobile-score") door data werd gefalsifieerd én transparant geaccepteerd zonder rationalisatie. Decisional-thresholds-tabel met 6 signalen × 3 frames vooraf in plan-file vastgelegd voorkwam dat ik na data-collectie naar een gewenste frame-conclusie kon driften. Defense-in-depth-persistence-pattern (Sessie 140) uitgebreid naar no-action-uitkomsten: TASKS.md item-closure + inline comment box-utils.js + audit-doc §2 closure-tabel. Bulk-rotation Sessie 145: 135/136/137/139 entries verwijderd uit CLAUDE.md, blijven in current.md. Volgende bulk Sessie 150.)

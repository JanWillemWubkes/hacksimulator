...
**Laatst bijgewerkt:** 2 jun 2026 (Sessie 148)
**Status:** M7 Gamification ✅ 100% | M6 Tutorial System 88% | M5.5 Monetization ✅ Live + Brevo deliverability tuned + Gumroad v1.0 + Lead magnet (LIVE on hacksimulator.nl) | Doc-protocol refactor + forcing function (Sessie 140) | Terminal Core ⚠️ ~547 KB minified + ✅ **Lighthouse terminal Mobile 49→59 / Desktop 77→94** ná Pad C1+C2 (Sessie 144, sample-pentest mobile 73→82 / desktop 99→100) + Sessie 147 baseline mediaan mobile 74 / FCP 1277 ms / LCP 4116 ms (run-variance lager dan Sessie 145/146) + Sessie 148 dedupe-fix item #31 (main.js single-fetch bewezen via Playwright Resource Timing API, ~4,6 KB cold-load besparing)
**Sprint:** Sessie 148: Item #31 ✅ gesloten — deterministische bug-fix `terminal.html:43` modulepreload version-param-mismatch (`src/main.js` synced naar `src/main.js?v=88-multiline-wrap`, conform bestaand `?v=114` CSS-pattern regels 41-42). Quick-win-pad ná 3 Frame B/C/D sessies (145/146/147). Verificatie via Playwright MCP `performance.getEntriesByType('resource')` filter: **pre-fix count: 2** (modulepreload `initiatorType: "other"` + script-tag `initiatorType: "script"`, beide 2323 bytes encoded) → **post-fix count: 1** (alleen `initiatorType: "other"` want script-tag hergebruikt via byte-exact URL-dedupe). Commit `12a93a2` + Netlify-deploy 36 sec (sneller dan typische 60-90 sec range). Pipeline-exit-code-discipline uit Sessie 147 toegepast (`set -o pipefail`). Spot-check Chromium-only `performance.spec.js`: 2 failures + 1 flaky onthuld via isolated re-run als **pre-existing flakes** (Load time < 3s passed in isolated re-run 2.69s, VFS NaN-failure is test-code-bug regel 496 `stdDev/avgGrowth` waar avgGrowth=0 → 0/0=NaN, niet door deze patch geïntroduceerd — causale onmogelijkheid want geen storage-codepath geraakt). Sessie 147 stash-verify-pattern direct herbruikt als spot-check discriminator. Artifacts `/tmp/sessie148-item31/{baseline-pre,baseline-post,verdict}.json`. Defense-in-depth 4 plekken (item #31 closure + current.md Sessie 148 + CLAUDE.md learnings + plan-file `/home/willem/.claude/plans/heisenberg-hier-cold-start-sessie-serialized-gadget.md`).
...
30. [ ] **Sync-inline navbar/footer HTML compile-time pre-render** (Sessie 147 spawn uit #29 Frame C closure) — Modulepreload verschuift alleen *fetch*-deel; parse + execute + injectie blijven binnen long-task #1 cascade. Echte cascade-elimination vereist DOM al statisch aanwezig vóór JS draait. **Hypothese:** sync-inline navbar+footer HTML reduceert mobile long-task #1 met >40% (>800 ms van 2000 ms mobile-throttled baseline) door cascade-window-elimination — geen JS nodig voor structurele DOM, init-components.js wordt enrichment-only ipv injection-only. **Bestaand pattern als bewijs-van-haalbaarheid:** terminal.html:82-94 noscript navbar + regel 370-371 noscript footer tonen dat statische reproductie haalbaar is. **Verify-first-vereisten:** (a) eigen plan-file met decisional-thresholds-tabel + signaal-clustering vooraf, (b) verse 3-run mediaan Lighthouse baseline (huidige main staat — niet hergebruik Sessie 147), (c) build-time render-stap (mogelijk Eleventy partials of vanilla JS pre-render-script naar terminal.html.template → terminal.html), (d) verse 3-run mediaan ná inline + bevestiging dat injectNavbar/injectFooter geen DOM-duplicaten triggert, (e) Playwright suite groen + visual regression check. **Scope:** ~3-4 uur (build-pipeline-impact, hogere risico-profile dan #29). **Eervolle Frame B-pad ingebouwd:** als sync-inline geen meet-bare cascade-elimination geeft (long-task #1 niet >40% gekrompen), accepteer ná-data en spawn #32 voor andere structurele paden (HTTP/2 server-push deprecated, self-host fonts, navbar/footer tree-shake).
...
**Laatst bijgewerkt:** 2 jun 2026 (Sessie 148)
**Versie:** 5.2 (Sessie 148 — Item #31 ✅ gesloten via deterministische bug-fix `terminal.html:43` modulepreload version-param-mismatch. Quick-win-pad ná 3 Frame B/C/D sessies (145 box-utils + 146 #28 + 147 #29): Heisenberg's keuze optie (b) — sync `?v=88-multiline-wrap` naar modulepreload, conform `?v=114` CSS-pattern regels 41-42. **Verificatie via Playwright MCP Resource Timing API:** pre-fix `count: 2` met entries `main.js` (initiatorType "other" = modulepreload regel 43) + `main.js?v=88-multiline-wrap` (initiatorType "script" = script-tag regel 385), beide 2323 bytes encoded / 8585 bytes decoded → bewijs dubbele cache-key-fetch op identieke file-content. Post-fix `count: 1` met enkele entry `main.js?v=88-multiline-wrap` (initiatorType "other" — modulepreload triggered, script-tag hergebruikte via byte-exact URL-dedupe). Mechanisme bewezen werkend zoals fetch-spec voorschrijft. Commit `12a93a2` + Netlify-deploy 36s. **Spot-check `performance.spec.js` Chromium-only:** 2 failures + 1 flaky alle 3 onthuld als pre-existing flakes via Sessie 147 isolated-rerun-pattern (Load time re-run passed 2.69s; VFS NaN test-code-bug regel 496 wegens 0/0 — onmogelijk patch-induced want geen storage-codepath geraakt; ES6 cascade marked flaky maar passed). Pipeline-exit-code-discipline `set -o pipefail` op deploy-poll direct toegepast (Sessie 147 leerpunt). Spawn #32 VFS-growth-test NaN-edge-case fix. **Audit-merit Sessie 147 aangetoond:** ondiepe audits hadden de URL-mismatch nooit gevonden; multi-bron LH-JSON-parse `network-requests`-section was detectie-trigger. Besparing ~4,6 KB transfer/cold-load + 1× v8.parseModule + v8.compileModule cycle. Page-perf-delta verwacht onder run-variance-noise (geen 3-run LH@11 mediaan want effect te klein). Defense-in-depth 4 plekken: TASKS.md item #31 + current.md Sessie 148 + CLAUDE.md learnings + plan-file `/home/willem/.claude/plans/heisenberg-hier-cold-start-sessie-serialized-gadget.md`. Artifacts `/tmp/sessie148-item31/{baseline-pre,baseline-post,verdict}.json`.)

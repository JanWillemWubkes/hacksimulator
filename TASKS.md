...
34. [ ] **Mechanism-isolation onderzoek voor Sessie 151 #27 Frame C variance-amplification** (Sessie 151 spawn uit #27 Frame C closure). Hypothesis: preconnect-only veroorzaakte de variance-amplification (early-connection-opening = early AdSense-backend-dependency-cascade), inline-CSS-only is timing-neutraal (puur source-growth). Verify-first scope: 2 separate cycli — (a) preconnect-only patch op 17 ad-bearing pages + verify; (b) inline-CSS-only patch op 17 ad-bearing pages + verify. Bij Frame B (NOISE) op inline-CSS-only én Frame C (regressie) op preconnect-only = hypothese bevestigd, inline-CSS toepasbaar als low-impact-low-risk pattern. Bij Frame B op beide = orthogonale variance-bron (Brevo backend variance op Index? AdSense-Auto-ads-state-variance?). Bij Frame C op beide = onbekende interactie tussen patches die alleen samen veroorzaakt. Scope: ~4-5 uur (2 cycli × ~2u). Volg na #34 verkenning of resource-hint optimizations op deze site überhaupt levenbaar zijn — anders documenteren als "geen verdere resource-hint experiments op ad-bearing pages, focus op andere mechanism-paden zoals #33 (b/c/d)".

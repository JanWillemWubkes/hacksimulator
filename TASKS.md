...
**Laatst bijgewerkt:** 15 jun 2026 (Sessie 168)
...
**Sprint:** Sessie 168 — Blog-tabel-uitlijning fix: blog-tabellen hadden geen CSS in `styles/blog.css` en vielen terug op browser-default `vertical-align: baseline`; bij afgebroken code-cellen zakte de beschrijving t.o.v. de filter → rijen uit sync. Scoped `.blog-post-content table/th/td`-blok toegevoegd met `vertical-align: top` als kern-fix + `border-collapse`/padding/rij-randen (patroon uit `legal.css`, blog-CSS-variabelen → thema-aware). Repareert alle 4 blog-tabellen (wireshark/nmap/hashcat/wachtwoord-beveiliging). Browser-geverifieerd: alle rijen `filterTop == descTop` in dark/light/375px, geen overflow. Commit `4368bb4`. **Pre-Sessie 168:** zie `docs/sessions/current.md`.

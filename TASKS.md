...
**Laatst bijgewerkt:** 15 jun 2026 (Sessie 167)
...
**Sprint:** Sessie 167 — Doc-drift fix M9: het Sessie 162 esbuild content-hash post-launch-blok (`### 🔵 OPEN (post-launch)`, 5 `[ ]`-items) is uit de M9-sectie verplaatst naar `## 🔮 Post-MVP Features`. Oorzaak: het blok zat als h3 binnen de Check 6 awk-range `/^## 🧹 M9:/,/^## 🎓 M6:/` → `--deep` telde 19/24 (79%) terwijl de tabel 19/19 (100%) zegt; fast-mode (pre-commit-gate) telt geen sectie-checkboxes en miste dit. Besluit (b): esbuild = nieuwe post-launch scope (raakt PRD §13 "no build step" red line, eerste taak = scope-besluit), GEEN M9-taak — M9 was legitiem ✅ Voltooid sinds Sessie 110 (`Total Tasks: 19`, 6 sub-secties × allen `[x]`). Move via Python met occurrence-asserts (h3-uniciteit, 5×`[ ]` behouden, M9-range 0 open, totaal-`[ ]` ongewijzigd). Gate: `validate-docs.sh --deep` exit 0, M9 nu 19/19. Doc-only, geen site-code, bundle onveranderd. **Pre-Sessie 167:** zie `docs/sessions/current.md`.
...
38. [x] **Pre-launch security-audit + hardening (Sessie 166)** — CSP `script-src` 'unsafe-inline'/'unsafe-hashes' verwijderd via inline-script-externalisatie naar /src/*.js (consent-default/brevo-config/init-theme/load-animations-css) + AdSense-race gesloten + X-XSS-Protection→0 + history.search() ReDoS-fix + privacy.html feitfout + frame-src/img-src `adtrafficquality` (F6) + security.txt (RFC 9116) + SECURITY.md. Geverifieerd: nul CSP-violations (4 page-archetypes onder echte CSP) + E2E 183 passed. Branch `security/csp-hardening-audit`, commit `aa0396d`. **Open:** merge naar `main` na Netlify-deploy-preview-check = productie.

...
**Sprint:** Sessie 144: Pad C1+C2 implementatie voltooid (item #24 ✅) — adsbygoogle.js verwijderd van 6 no-slot pages (terminal/sample-pentest/gidsen/3 legal-pages) + animations.css critical-split op terminal.html + fetchpriority hints. AdSense ecosysteem 252 KB / 420 ms → **0/0** op productie (alle 4 Lighthouse runs). Sample-pentest desktop nu 100/100. Eerlijk-flag: terminal mobile 59 onder verwacht (70-80) door first-party bottleneck box-utils.js (item #26 prioriteit verhoogd). Nieuw item #27 voor ad-bearing pages perf-audit.
...
26. [ ] **`box-utils.js` bootup-time profile** (prioriteit verhoogd Sessie 144 want terminal mobile-score 59 wordt nu door first-party bottleneck gedomineerd ná third-party cleanup) — Lighthouse `audits["bootup-time"]` op `/terminal.html` mobile rapporteerde `src/utils/box-utils.js` op **309 ms total / 200 ms scripting** = #3 op de bootup-tabel direct na AdSense ad-slot (915 ms, nu weg ná Pad C2) en adsbygoogle.js (409 ms, nu weg). Verrassend hoog voor utility-code (vergelijk: main.css 102 ms / 0 ms scripting). Hypotheses te onderzoeken: (a) `measureCharWidth()` met `getComputedStyle` + Canvas API draait herhaaldelijk zonder cache-hit (regels 12-50), (b) cache-key `fontSize|fontFamily` faalt bij `font-display:swap` font-substitutie, (c) box-render-loops over groot output-volume. Scope: ~30 min profile (Chrome DevTools Performance tab) + cache-warming patch indien (a)/(b) bevestigd. Verwachte impact ná patch: +5-15 mobile-score (gebaseerd op TBT-deel dat box-utils nu domineert). Defense-in-depth persistence: dit task + inline TODO bovenaan `src/utils/box-utils.js` regel 1.
...
**Laatst bijgewerkt:** 29 mei 2026 (Sessie 144)
**Versie:** 4.8 (Sessie 144 — Pad C1+C2 implementatie voltooid via commit `4e4eec5`. Scope-uitbreiding via cold-start grep: 6 no-slot pages (terminal/sample-pentest/gidsen/3 legal) krijgen adsbygoogle.js verwijderd. Animations.css critical-split op terminal.html (inline a11y + modal-fade-in, defer rest). fetchpriority="high" op preloads. Productie-Lighthouse delta: terminal mobile **49→59** / desktop **77→94**, sample-pentest mobile **73→82** / desktop **99→100**. AdSense ecosysteem 252 KB / 420 ms → **0/0** op alle 4 runs. Eerlijk-flag: terminal mobile onder verwacht 70-80 door first-party bottleneck box-utils.js (item #26 prioriteit verhoogd). Nieuw item #27 voor ad-bearing pages perf-audit. Item #24 ✅ voltooid.)

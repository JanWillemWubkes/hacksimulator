...
- **Playwright:** 34 spec files, 274 `test()`-declaraties per browser-project (Sessie 216: netto +1 in `homepage-conversion.spec.js` — de guard-loop bleef 3 geparametriseerde tests maar kreeg er een 4e assertie bij, plus NEW "de knop klapt mee met zijn balk"; `hero-demo.spec.js` ging van 2 naar 3 chipmaten). Bundel-limiet 1100 KB, gemeten **1084,92 KB** — marge 15,08 KB (1,4%). Die grens meet ongeminificeerde broncode sitebreed en is een **driftalarm, geen perf-poort**: Terminal Core (<400 KB minified, de echte poort) wordt met nul bytes geraakt. ⚠️ Bij de volgende CSS-toevoeging is de vraag "klopt 1100 nog", niet "hoe knip ik bytes".

<!-- VALIDATE-BUNDLE-START Sessie 157 — ground-truth target voor scripts/validate-docs.sh --deep -->
<!-- src=720 styles=429 blog=474 assets=1737 (KB unminified, du -sb / 1024 basis; Sessie 217 ground-truth meting) -->
<!-- VALIDATE-BUNDLE-END -->

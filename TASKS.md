...
**Laatst bijgewerkt:** 28 jul 2026 (Sessie 202; mobiele kolom-uitlijning + box-truncatie fixes)
**Sprint:** Sessie 202: **Mobiele kolom-uitlijning + box-truncatie in terminal-output (1 commit `fe27a17`, gepusht)** — n.a.v. gebruikersscreenshot: het `reset`-tutorial-exit-menu brak lelijk af op 375px (twee zware kolommen commando+beschrijving vielen onder elkaar → beschrijving las als los menu-item); verzoek: sitebreed analyseren + perfect maken. **Root cause:** `#terminal-output` gebruikt `white-space: pre-wrap` → regels breder dan ~30 tekens breken af op interne spaties, waardoor fixed-width twee-koloms-tabellen in elkaar klappen; de `data-indent` hanging-indent redt alléén continuatie-regels. De codebase had het juiste `isMobileView()`-patroon al (`next`/`tutorial`/`dashboard`/`leaderboard`), alleen inconsistent toegepast. **Scope-keuze (Heisenberg): alle live output incl. security-tools; man-pages buiten scope.** Fixes: (1) `reset.js`-menu gestapeld (commando + ingesprongen beschrijving + blank-line-groepering; géén per-regel-arrow want dat overschreed de 30-char-breedte); (2) `nikto.js` "Ontbrekende headers"-tabel gestapeld + **2 echte bugs** (output-regels zonder `\n` aan elkaar geplakt: Start Time/Server, End Time/1 host) + "Forces HTTPS"→"Dwingt HTTPS af"; (3) `hashcat.js` 2 lange kolom-0-regels met ←-glosse → glosse op eigen ingesprongen regel; (4) **`asciiBox.js` box-`wrap()` woord-wrapt te brede regels i.p.v. afkappen** — de échte reikwijdte-vondst: de 5 SECURITY WARNING-boxen kápten waarschuwingstekst af op mobiel (niet alleen hydra); regels die passen blijven verbatim → desktop ongewijzigd. **Follow-up-vondst:** latente `RangeError` in box-titel (`repeat(negatief)` bij titel > boxbreedte) — onbereikbaar in prod (beide callers man/help zijn desktop-only) maar goedkope titel-afkap-guard toegevoegd (crash ≫ cosmetisch). `metasploit.js`/`hydra.js` glosse-regels bewust ongemoeid (al ≥3 spaties → hanging-indent grijpt al aan). **Verificatie:** Playwright 375px + desktop (0 overflow, randen uitgelijnd, geen desktop-regressie); box-fix getest via verse `import('...?cb=')` want geen build-stap + `asciiBox.js` heeft geen `?v=` → browser-modulecache serveerde de oude versie (in-app render toonde stale; de directe module-import raakt het échte on-disk bestand). **Deploy-noot:** live na ~1u Netlify-cache (sub-modules relatief geïmporteerd zonder `?v=`; harde purge = "Clear cache and deploy site"). Volledig: `docs/sessions/current.md` Sessie 202. **Vorige sprint:** Sessie 201: **Koppen sitebreed naar Nederlands zinskapitaal i.p.v. Engelse Title Case (1 commit `65c5f18`, gepusht)** — n.a.v. gebruikersmelding dat blog-titels inconsistent gekapitaliseerd waren (Engelse Title Case, bijv. "Payloads: Wat Gebeurt er Na de Exploit?"). 2 Explore-audits: sitebreed ~340 koppen in Title Case (15 blogs + 8 hoofdpagina's + 3 juridische pagina's). Omgezet naar zinskapitaal (1e woord + eigennamen) via zelfgeschreven Python-transform (`sentence_case.py`, scratchpad) met eigennamen-whitelist + zin-grens-detectie (na `?`/`!`/`.` = hoofdletter, na `:` niet, afkortingen "vs."/"art." geen valse zin-grens, sectienummers "2.1" wél); **27 bestanden, 518/518 gebalanceerde regel-vervangingen** (puur tekst-casing, 0 structuur). Per blog-artikel bewegen `<title>`/og/twitter/JSON-LD headline+breadcrumb-name/`<h1>`/blog-index-kaart in **lockstep** (geen drift gestructureerde data ↔ zichtbare kop). Behouden: merken (Metasploit/Nmap/OWASP **Top 10**), nationaliteitsadjectieven (Nederlandse/Engelse), camelCase-API's (localStorage), acroniemen (TCP/WAF/GDPR/SMS), officiële namen (OWASP-categorieën A01–A10, certificeringen "Certified Ethical Hacker"), juridische afk. (III/Sr), toets-notatie (Ctrl+C), command-namen (klein — ook `<h2>nmap</h2>` op commands-pagina overgeslagen). Engelse vaktermen ("brute force"/"social engineering") wél klein (Heisenberg-keuze: zuiver zinskapitaal). **Kritieke script-valkuil ondervangen:** `<style>`/`<script>` gemaskeerd vóór de heading-regex — woordenlijst.html heeft letterlijk `<h3>` in een CSS-comment → runaway-match zou style-blok + JSON-LD verminken; JSON-LD apart met string-patronen verwerkt. Verificatie: idempotentie-check (2e pass = 0 wijzigingen) + `git diff` collateral-scan leeg + browser-render metasploit-blog + pre-commit blog-JSON-LD/tag-balans-hook groen. Legal-modalkop `src/ui/legal.js` handmatig mee. **+typofix** "Scheibaarheid"→"Scheidbaarheid" (`terms.html:301`, severability-clausule). Bewust: "Filesystem commands" e.d. óók klein (consistenter dan plan-"niet aanraken"-noot); "Probeer het Metasploit command" merk-hoofdletter. NEW memory `feedback_dutch_sentence_case_headings`. Volledig: `docs/sessions/current.md` Sessie 201. **Pre-Sessie 201:** Sessie 200: **Command-output-audit (item #47) afgerond — Fase 3 triviale correctheid-nits (1 commit `e31075b`, gepusht)** — n.a.v. "voer Fase 3 uit: drie triviale correctheid-nits, in één commit naar main". Sluit item #47 dat op 26 jul in een aparte werkblok-reeks liep: metasploit-command-cleanup (`f56d886`) → 3-fasen-roadmap (`ebc86c8`) → Fase 1 security-cluster sqlmap/hydra/hashcat/nikto (`f65c93d`) → Fase 2 meta-funnel next/shortcuts/onboarding/help (`0a256ad`) → **Fase 3 (deze conversatie):** `cp.js:47` "De bronbestand"→"Het bronbestand" (onzijdig, consistent met `mv.js:61`); `certificates.js:95`+`:158` "kopieren"→"kopiëren" (trema, `replace_all`, grep bewees exact 2 voorkomens in `src/`); `nmap.js:160`+`:234` onvertaald "attack surface"→"aanvalsoppervlak" (onzijdig "minimaal", term die manPage `:220` zelf al glosst). Bewust NIET: `nmap.js:53` code-comment + `:162`/`:235` "meer ingangen"-frasering (ander register, slecht-geval). Geverifieerd: `node --check` 3 bestanden + browser-driving via directe module-import (`cp.execute` met vfs-stub, `nmap.execute` op `secure-*`-target voor de 443-only-TIP, beide manPages) → alle 4 user-facing strings correct incl. `ë`-UTF-8; `certificates.js:158` code-inspectie (challenge-gated). **Pre-Sessie 200:** Sessie 199: **Marketing-launch uitvoeren — verse launch-week-post + launch-beslissing (5 commits, gepusht)** — n.a.v. "ik wil de marketing launch uitvoeren; zeg me precies wat te doen en of ik nu kan launchen". Diagnose: technisch launch-ready, alles voorbereid (Sessie 161-198). **Heisenberg-besluit: demand-validatie (item #44) bewust overslaan → direct launchen; doel wo 29 juli 2026.** Gebouwd (Deel A, code/content live): (1) **NEW `blog/metasploit-beginnersgids.html`** — verse launch-week-post (freshness-hefboom, runbook Fase 2), beginnersgids over het exploit-framework (bouwstenen/workflow/payloads/patch-leermoment/juridische grens art. 138ab Sr), gemodelleerd op nmap-post (JSON-LD Article Organization-auteur + BreadcrumbList, blauw palet, consent-model-CTA's); datePublished 22 jul (echte publicatiedag, geen future-dating). Ingehaakt in blog-index (nieuwste boven) + feed.xml (RSS newest-first + lastBuildDate) + sitemap + homepage-bloglinks. (2) **Cornerstone-touch:** natuurlijke interne links naar de nieuwe gids vanaf `cybersecurity-tools.html` (noemt Metasploit al) + `nmap-beginnersgids.html` (recon→exploitation); beide `article:modified_time` + JSON-LD `dateModified` + sitemap `lastmod` eerlijk naar 22 jul; homepage-lastmod mee. (3) **Admin/besluit gelogd:** #44 → BEWUST OVERGESLAGEN (poort niet weggeredeneerd; launch-data vervangt validatiesignaal), #45 (value-prop/retentie) gaat nu op echte launch-data, launch-uitvoering-item gedateerd (29 jul, D-1/dag/D+14). (4) **NEW `docs/launch-checklist.md`** — één vindbare stap-voor-stap to-do (Blok 1 deze week / Blok 2 D-1 28 jul / Blok 3 launch-dag / D+14), verwijst naar announcement-kit/success-metrics/runbook; TASKS-launch-item wijst er als eerste naar. Geverifieerd: validate-blogs 15/15, validate-docs --deep 9/9, XML well-formed, render-en-meet dark/light/375px 0 overflow + alle interne links 200. Bundle blog 415→447 KB (marker gesynct). Commits `f6894b1`+`4245ec9`+`7f65d94`+`d45631d` (+ deze doc-sync), gepusht naar `main`. **Deel B (Heisenberg, handmatig):** `docs/launch-checklist.md` afwerken. Volledig: `docs/sessions/current.md` Sessie 199. **Pre-Sessie 199:** Sessie 198: **Launch-readiness: 3 workstreams (funnel-meetbaarheid + demand-validatie + value-prop/retentie)** — n.a.v. de vraag "wat is iets fundamenteels dat we nog moeten doen voor de (uitgestelde) marketing-launch?". Brutaal-eerlijke analyse: het product is technisch launch-ready maar strategisch **on**gevalideerd (nul extern signaal, geen vooraf-gedefinieerd succescriterium). Gebruiker koos "alles aanpakken" (goedgekeurd plan). **WS1 — Launch meetklaar (code live, 4 commits):** analytics-funnel-audit (`events.js`/`tracker.js`/`consent.js`): de meeste events waren al bedraad (session→tutorial→challenge→newsletter/lead-magnet→product-CTA), maar **2 launch-kritische ontbraken**. Gebouwd: **`terminal_cta_click`** (`commit 4728c54`) op de 6 primaire homepage→terminal-CTA's (hero/mid/final + 3 leerpad-deeplinks) via `data-terminal-cta`-attr + branch in `cta-tracking.js` (reuse delegated-click-patroon) — meet de #1 conversie per locatie; **`terminal_activated`** bij de eerste succesvolle command (once-per-tab-sessie via sessionStorage-guard) — de activation-rate. Bewust géén nav-link-tracking (zat in `<noscript>`, kan met JS-vereiste analytics nooit vuren → dode attr weg) + geen gedeelde geïnjecteerde navbar (lage waarde). Geverifieerd via browser-self-test (consent pre-set, funnel gedreven, dataLayer-pushes opgevangen): **alle funnel-events vuren, 0 ontbrekend**; activation exact 1×/sessie + niet na reload; 0 code-console-errors (alleen egress-geblokkeerd GA4-script faalt, laadt in prod prima — dataLayer-shim werkt). 56 e2e groen (command/gamification/tutorial — kern-flow intact). NEW `docs/launch-success-metrics.md`: vooraf-vastgelegde succesdefinitie (funnel→event-mapping, streefgetallen als expliciete hypotheses, dag-1-minimum, GA4-config-stappen incl. key-events + funnel-exploration + custom dimensions + DebugView-check, read-order launch-dag; north-star = activation-rate). Cache-bump `main.js v=201→202-funnel` + `init-components.js v=3→4` (24 pagina's). **WS2 — Demand-validatie (doc):** NEW `docs/demand-validation-protocol.md` — 5-taken cold-start-test (5-sec-positioneringstest → CTA-keuze → activation → missie → deel/terugkeer-intentie) + drop-off-checkpoints + 5 na-vragen + resultaten-template + recruiting voor NL-beginners (mijd HN/netsec — vertekent het signaal). **WS3 — Value-prop + retentie (doc):** NEW `docs/value-prop-and-retention.md` — brutaal-eerlijke hero-audit (correct maar niet magnetisch) + 3 kandidaat-hero's om te toetsen + retentie-opties gerangschikt. **Bewust NIET live gezet** — hero-copy + retentie-systeem wachten op het WS2-validatiesignaal (nu op eigen smaak gokken = exact de onbewezen-aanname-fout die de analyse aankaartte). **Inherent van Heisenberg:** GA4-goals configureren + DebugView-check; 5-10 validatie-sessies draaien; ná validatie hero-variant + retentie-optie kiezen → dán bouw ik. Alle 4 code+doc-commits ff-gemerged naar `main` (`efdf958..23bbb1a`, gedeployed). **Stop-hook signing-noot:** commits tonen als "Unverified" — niet door e-mail (al correct `noreply@anthropic.com`) maar door ontbrekende SSH-handtekening; de geconfigureerde signing-key is een leeg 0-byte-bestand + draai als root zonder toegang tot `claude`-keys → niet ondertekenbaar in deze omgeving; bewust niet geforceerd (amend/rebase voegt geen handtekening toe + herschrijft gedeployde main-historie voor niets). Volledig: `docs/sessions/current.md` Sessie 198. **Pre-Sessie 198:** Sessie 197: **Laatste volledige simulator-bug-test + 2 fixes (3 commits, gepusht)** — n.a.v. gebruikersvraag "test functies en flow 1 laatste keer op bugs" (scope: alléén de terminal-simulator; beleid: bevestigde bugs direct fixen). Aanpak: 8 systematische browser-driving-passes met echte `fill`+`Enter` (nooit synthetische events — Sessie 196-les), tegen de lokale werkkopie op verse poort. **Dekking:** alle 41 commands (happy+error-paden) + 41 man-pages + typo-fuzzy; security-consent-flow alle 5 tools (warning→consent→gedeeld→`reset consent` re-armt, metasploit no-arg-variant); 5 tutorials end-to-end + exit/resume + reload-mid-missie + herstart + cert; challenges easy/medium/hard end-to-end + order-recovery (sql-sleuth/attack-chain locken niet) + reload-resume + tutorial⇄challenge mutual exclusion; gamification (leerpad-vinkjes, hasError-guard laat gefaalde traceroute níét afvinken, EXPERT-unlock ≥4-van-6, dashboard/achievements/certificates/leaderboard/next/shortcuts); core-input (VFS-persistence over reload, `reset`-herstel, ↑↓-history, Tab cmd+pad, Ctrl+R/L/C); welcome state-aware; mobile 375px. **1 echte bug + 1 robuustheidsfix:** **(1) `3d7df13` mobiele 10px horizontale overflow** — `#terminal-container` erfde `width:100%` uit de basis maar kreeg in de mobiele media-query `margin:10px` → met border-box rechterrand op 385px voorbij de 375-viewport (`body overflow-x:hidden` clipte het = onzichtbaar, maar terminal 10px uit het midden + rechter 10px content afgekapt; navbar rekte mee naar 385). Pre-existing (Sessie 189 noteerde de "10px offset" zonder oorzaak). Fix `width:auto`; render-en-meet 375px 0 overflow + symmetrisch gecentreerd (10px beide zijden), desktop 1280 ongewijzigd. Cache-bump `mobile.css v=115→116` (24 HTML-refs). **(2) `9bd487b` persistence-flush** — progress-store + persistence flushten hun 500ms-debounce alleen op `beforeunload` (op mobiel/iOS Safari onbetrouwbaar bij app-switch/scherm-lock) → een challenge voltooien én binnen 500ms de tab backgrounden verloor de voltooiing (`completeChallenge` schrijft via de debounce). Fix: +`pagehide` +`visibilitychange(hidden)` → `flush()` in beide stores (idempotent). Bewezen: challenge voltooid + `<500ms` visibilitychange(hidden) → voltooiing staat in localStorage (stond er zonder de flush nog niet); idem VFS-mutatie. NEW `persistence-flush.spec.js` (2 tests) → 28 spec-bestanden. Cache-bump `main.js v=199→200-persist-flush`. **~6 vals-positieven onderzocht + verworpen** (heuristiek-matches op body-tekst, progress-store 500ms-debounce vs snelle localStorage-lees, scrollback-accumulatie in `innerText`, by-design typewriter-tap-guard); **modal-protection/focus-trap expliciet geverifieerd degelijk** (echte gebruiker kan niet in de terminal typen met legal-modal actief; het eerste "bug"-signaal was een `fill()`-focus-artefact). **Bewust NIET (2× brutaal-eerlijk advies):** (a) 10 prod-hardcoded specs → `BASE_URL` = **backlog item 42** (testinfra-schuld, geen gebruiker-bug, per-bestand-oordeel nodig want performance/debug wijzen mogelijk bewust naar prod; hoogste-waarde-move = smoketests van werkkopie-tests splitsen); (b) scope niet uitgebreid naar entry-points/hele-site (deep-link al spec-gedekt, blog/consent/lead-magnet net ~15 sessies geaudit → uitbreiden verwatert de finale check). Werkkopie schoon, 184 werkkopie-specs groen vóór én na de fixes (de 49 "failures" = 10 prod-hardcoded specs egress-`000` + 5 tutorial-gestures device-emulatie-harnasartefact — geen werkkopie-regressie). Volledig: `docs/sessions/current.md` Sessie 197. **Pre-Sessie 197:** Sessie 196: **CTA-consistentie-audit + fixronde (4 commits, gepusht)** — n.a.v. gebruikersvraag "tutorials zeggen soms 'typ next', soms een directe opdracht — bewust of bug?". **Auditverdict (2 Explore + 1 Plan-agent): bewust design, geen bug** — alle 40 tutorial-stappen schrijven een commando voor en advancen alléén op correct commando (`validate()`); "Typ 'next'" leeft uitsluitend in de vrije-verkenning-funnel (onboarding/welcome/completion); de Sessie 190/193-guards staan aantoonbaar in terminal.js. Wél 4 klassen echte drift gevonden + gefixt: **(1) `9532b0b` NL-copy:** ~31 Engelse Type→Typ, waarvan 24 hint-strings in álle 5 scenario's ('Type het commando'/'Type: <cmd>' — de Sessie-193-sweep miste het hints-array-patroon) + reset/help-system/help/shortcuts-manpage ('Type Ctrl+R'→'Druk'), quote-unificatie double→single, search-placeholder '🔍 Type to search...'→'Zoek een command...' (NL + emoji-regel), terminal.html edu-steps 2×. **(2) `914677d` marker-unificatie:** dezelfde CTA "Typ 'next' voor je volgende stap" had 3 markers ([→] / kale → / [?] in onboarding-progressive-hints/leerpad/dashboard) → overal `[→]` = primaire actie-CTA (hiërarchie Sessie 191-195); instructie-bullets blijven bewust kale pijlen; renderer kleurt alle drie info → visueel no-op; leerpad-padEnd-box gemeten pixel-uitgelijnd (38 regels × exact 1148px). **(3) `43eeb58` dubbel-marker:** `[?] TIP:` → canoniek `[TIP]` (82 hits, 27 bestanden; `_stripTips` matcht al beide vormen — dual-match blijft als vangnet; geen e2e-contract geraakt). **(4) `7fce8c5` twee gedragsbugs:** (a) welcome-CTA/placeholder niet challenge-aware — hervatte challenge op boot toonde `[→] Typ 'next'` + 100ms later 'Challenge hervat — typ challenge status' = 2 concurrerende instructies → `ctaMode`-suppress + placeholder-flip óók op `challengeManager.isActive()` (resume() draait in init() vóór de welcome-render); (b) missie-commands verbruikten one-time-tips (Tab/Ctrl+R) stilletjes — `recordCommand()` passeerde de exacte `===`-drempels en consumeerde flags terwijl terminal.js de hint nulde → NEW `recordCommand(cmd, {deferHints})` bevriest commandCount + hints tijdens missies, `commandsTried` (leerpad-vinkjes!) wél bijgewerkt; nulling-guard vervalt. Bewust géén `===`→`>=` (copy-mismatch + hint-cascade). **Tests:** bug-J-reload uitgebreid (welcome-suppress + neutrale placeholder) + NIEUW challenge-completion-CTA exact 1× (spiegel fundamentals.spec) + NIEUW deferHints-state-assert → 27 specs / 238 tests (+2). **Volledige chromium-suite 0 failures** (230 passed, 3 bekende flaky-op-retry: 2× tab-autocomplete + 1× long-press-gesture, 5 skipped; verse poort 8199, BASE_URL). Render-en-meet: 375px geen overflow + [TIP]/[→] info-kleur + D1 live bevestigd (1920×1080). **Bewust NIET:** next.js ASCII `[->]`/`<-` (padEnd-uitlijning + next-funnel.spec-contract), kale →-lijst-bullets (opsommingsteken ≠ CTA), EN-vakjargon in tool-output (hash type e.d., 80/20), usage-syntax-vormen ('Gebruik: challenge start <id>'). Cache-bump `v=199-cta-audit`. Volledig: `docs/sessions/current.md` Sessie 196. **Pre-Sessie 196:** Sessie 195: **leerpad-consistentie + brede spook-command-nasweep (3 commits, gepusht)** — n.a.v. gebruikersvraag "leerpad toont niet alle commands (whois ontbreekt)" + follow-up "wat missen we nog?". **Deel 1 (`c9ffc65`):** leerpad rende­rde een hardcoded 22-command-subset terwijl tracking álle correct-gebruikte commands registreert → whois (geleerd in `tutorial recon`!) kreeg nergens een vinkje. NEW `src/core/learning-path.js` = single source of truth (phases/tiers/`PHASE3_UNLOCK_THRESHOLD`); 6 consumenten (leerpad/next/dashboard/help/dynamic-content/onboarding-gate) importeren nu i.p.v. eigen kopie. Fase 2 +find/grep (8), Fase 3 +whois/traceroute (6, recon-volgorde). EXPERT-unlock ≥4-van-6 (backwards-compat met oude 4/4 + recon-finisher unlockt direct). Homepage-GEVORDERD-chips + manpage gecorrigeerd. **Deel 2+3 (`af2bd78` code, `6afd3d6` content/docs):** 3 parallelle audits (content-oppervlakken / in-app-data / docs) vonden 20 inconsistenties, dezelfde klasse. **Functioneel:** terminal.js `hasError` miste traceroute's `Failed to resolve` → gefaalde traceroute vinkte Fase 3 af (regressietest); `SIMULATOR_COMMANDS` één bron + hint/shortcuts toegevoegd (kregen geen `*`); badge network-novice → `phaseCommandNames(2)`. **Content:** terminal.html wireshark→traceroute (bestond niet); commands/index.html 39→41 (shortcuts+welcome ontbraken, JSON-LD+stats); blog-spoken ps/top/uname/curl/chmod eerlijk gemaakt; kale-command-CTA's conform consent-model. **Docs:** prd Bijlage A 30→41, style-guide [X]→[✓] checkbox-spec, commands-list voorbeelden. 74 E2E chromium groen (verse poort, `BASE_URL`) +2 regressietests. **Bewust NIET:** feed.xml OWASP "2021" (= officieel uitgebrachte editie, 2025 nog concept — audit-suggestie geverifieerd fout); help-system categorie-lijsten (correct, refactor te breed). Cache-bumps `v=197-leerpad`+`v=198-consistency`. Volledig: `docs/sessions/current.md` Sessie 195. **Pre-Sessie 195:** Sessie 194: **uitgestelde Sessie-193-punten beslist: 3 gebouwd + 4 document-and-accept** — value/effort-triage van de 7 bewust-uitgestelde punten. **Gebouwd (3 commits `380417e`+`8e7bbe6`+`f276820`, gepusht):** (1) analytics-guard: `tutorialEvent('completed')` vuurt alleen bij éérste voltooiing (replay telde dubbel; challenge-kant bleek al veilig — `start()` weigert voltooide challenges); (2) **VFS-schema-signature** op `hacksim_filesystem`: runtime djb2-hash over `JSON.stringify(initialFilesystem)` als `base`-veld (geen handmatige bump — kan niet vergeten worden; deterministisch want fase-content wordt bij lézen geïnjecteerd via dynamic-content.js) → stale save bij load verworpen + key opgeruimd + eenmalige `[~]`-notice; deploys die structure.js wijzigen bereiken nu terugkerende bezoekers. Bewust GEEN versievelden op de 4 JSON-state-keys (||default-tolerantie, geen consument = YAGNI); (3) `[TIP]` first-class info-marker: renderer-branch op BEIDE mapping-plekken (renderOutput + _renderLinesInto) → 37 tips cyaan met 1 regel, géén string-sweep (security-warnings ongemoeid); style-guide-conflict met CLAUDE.md/tone-and-output beslecht ([TIP]=educatieve tip, [?]=hint/verwijzing, beide info). **Document-and-accept (rationale in current.md + troubleshooting.md):** multi-tab last-write-wins (dagdeel reconcile-complexiteit vs zeldzaam+zelfherstellend), hint-tier-reset na reload (onzichtbaar; auto-hint-tier persisteert al), mobiel virtual-keyboard (needs real device), per-scenario-voortgangsmap (single-slot volstaat). NEW `vfs-versioning.spec.js` (3 tests) + `[TIP]`-kleur-assert; 62 chromium groen lokaal (`BASE_URL`, verse poort). Cache-bump `v=196-vfs-version`. Volledig: `docs/sessions/current.md` Sessie 194. **Pre-Sessie 194:** Sessie 193: **volledige tutorial-flow-audit + 18 fixes (A–P) in 4 commits** — n.a.v. 3 UX-klachten (deep-link toonde "Typ 'next'" naast "gebruik pwd"; deep-link naar andere missie toonde stale hervat-tekst; `[~]`/`[?]`-hint-inconsistentie). Audit (5 Explore + Plan-agents) legde 2 lagen bloot. **Begeleiding:** deep-link-id vóór welcome-render doorgegeven → state-aware CTA (`deeplink`/`suppress`/`default`) + resume-onderdrukking bij missie-wissel; `getState()`→`getStatus()` (deep-link naar zelfde missie wiste voortgang); onboarding-leaks (filesystem/simulator-hint) geguard; placeholder Type→Typ + state-aware. **State:** `resume()` laadt `completedScenarios` vóór early-return (voltooide missies raakten permanent kwijt na reload); `exit()` bewaart nu écht (active:false-slot, `tutorial <id>` hervat op stap); tutorial⇄challenge wederzijds uitgesloten. **Omgeving:** NEW `scenario-setup.js` normaliseert VFS bij verse start (cwd-drift + mkdir-"File exists"-strand + gewiste read-targets uit `initialFilesystem`); challenge overleeft reload (persist + resume); `clear` heroriënteert; mobiele ls/nmap-knoppen verborgen tijdens missie + typewriter-injectie dicht; exploitation-shadow-tekst eerlijk (permission-denied = leermoment); consent-writes gewrapt; 2 challenge-order-loks (sql-sleuth/attack-chain) niet-lockend. **Consistentie:** hint-markers geünificeerd; ~90 `Type '`→`Typ '` sweep; style-guide marker-tabel gecorrigeerd + command-checklist (j/n)-flow gecorrigeerd. 13 nieuwe e2e-asserts; volledige aangeraakte chromium-suite groen; browser-geverifieerd. Cache-bumps `v=194-deeplink-ux` + `v=195-env-robust`. Commits `c222597`+`2d93dae`+`606f85c`+`ced455d`, gepusht naar `main`. Volledig: `docs/sessions/current.md` Sessie 193. **Pre-Sessie 193:** Sessie 192: tutorial-voltooiing past in beeld — inline-certificaat (~20 regels) uit auto-voltooiing (blok 1,6× viewport verborg de next-step CTA onder de vouw); blok krimpt en past nu, scroll-anker terug naar `_scrollToBottom`; certificaat blijft op klembord + `tutorial cert`. Gemeten 1920×1080: commando-output én CTA beide zichtbaar
...
<!-- src=671 styles=394 blog=447 assets=1030 (KB unminified, du -sb / 1024 basis; Sessie 200 ground-truth meting — Fase 3 = tekst-only correctheid-nits, ~0 KB delta) -->
<!-- VALIDATE-BUNDLE-END -->
...
- [ ] **Launch-uitvoering — doel wo 29 juli 2026 (handmatig, demand-validatie #44 bewust overgeslagen).** → **Volledige stap-voor-stap to-do: `docs/launch-checklist.md`** (single source voor Heisenberg's handelingen). Kort: D-1 (di 28 jul avond) GSC sitemap-resubmit + indexering aanvragen homepage/`/blog/`/metasploit-post + Bing import + GA4 DebugView-funneltest + link-check copy; launch-dag GA4-annotatie + posten per `docs/launch-announcement-kit.md` §5 (blok 13:00–18:00 CET, EHGN→Reddit→Show HN→LinkedIn→X). D+14 `site:` + GSC Coverage her-meten.

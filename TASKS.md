...
42. [ ] **Test-suite: 10 prod-hardcoded specs → `BASE_URL` of eerlijk skip-signaal (backlog, Sessie 197)** — 10 spec-bestanden `goto`'en `https://hacksimulator.nl` of gebruiken `TEST_URL`/`PRODUCTION_URL`-constanten i.p.v. `baseURL` (feedback, autocomplete-filesystem, css-variable-test, debug-console, debug-storage, feedback-onboarding-headers, modal-colors-simple, modal-headers, performance, responsive-breakpoints). Gevolg: ~44 tests valideren nooit de werkkopie — ze hitten productie (andere versie) of falen bij geblokkeerde egress, wat traint om rood te negeren en zo een echte regressie in feedback/modals/autocomplete kan maskeren. **Niet blind alle 10 omzetten:** `performance.spec` (echte on-wire-meting) + `debug-*` wijzen mogelijk bewust naar productie → per bestand oordelen. **Hoogste-waarde-move (i.p.v. URL-swap):** splits "productie-smoketests" van "werkkopie-tests" (apart Playwright-project óf `test.skip` met heldere reden als het doel onbereikbaar is) zodat een lokale run een eerlijk groen-of-geskipt-signaal geeft i.p.v. een muur van rood. Testinfra-schuld, géén gebruiker-bug — losse scoped taak, lagere urgentie. Vastgelegd Sessie 197 (laatste simulator-bug-test). Let op: verificatie is lastig vanuit een egress-geblokkeerde omgeving — de omgezette specs zijn dan niet tegen productie te draaien om te bevestigen dat ze hun oorspronkelijke doel behouden.
...
44. [ ] **Launch-readiness WS2: demand-validatie uitvoeren (Sessie 198)** — protocol klaar (`docs/demand-validation-protocol.md`, 5-taken cold-start-test). **Open (Heisenberg):** 5-10 NL-beginners werven + sessies draaien + resultaten-template invullen; daarna samen interpreteren.
...
- [ ] **Launch-uitvoering op de nieuwe launch-dag (handmatig, datum ná item #44):** GSC sitemap-resubmit + indexering top-pagina's, Bing import, Rich Results/FB/X validators, GA4-annotatie, posten per `docs/launch-announcement-kit.md` §5

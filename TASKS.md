...
**Laatst bijgewerkt:** 6 jun 2026 (Sessie 152)
...
34. [ ] **Mechanism-isolation onderzoek voor Sessie 151 #27 Frame C variance-amplification** (Sessie 151 spawn uit #27 Frame C closure). Sub-pad **(a) ✅ CLOSED Sessie 152 Frame B NOISE-no-action REVERT** — zie hieronder. Sub-pad **(b) inline-CSS-only** blijft open voor Sessie 153+ verify-first cyclus.
  ...
  - **(b) Inline-CSS-only mechanism-isolation** — Open. Sessie 153+ verify-first cyclus. 6 critical-CSS selectoren uit terminal.html lines 53-59 verbatim copy naar 17 ad-bearing pages, GEEN preconnect. Discriminator-uitkomsten: Frame B (source-growth-only orthogonal aan variance), Frame C (inline-CSS = variance-amplifier was), Frame A (inline-CSS beneficial alone). Sessie 152 #34 (a) data wijst naar Frame B als waarschijnlijkste maar inline-CSS heeft eigen mechanism (paint-pipeline-elimination critical-style-blocking) dat preconnect niet heeft.

...
**Laatst bijgewerkt:** 12 aug 2026 (Sessie 221; vijf ongelogde commits — de regel die twee ervan stuurde, bleek zelf fout)
**Sprint:** Sessie 221: **Vijf commits over drie dagen, en de regel die twee van hen stuurde bleek zelf fout** (`0dd0c64` + `8c0c455` + `c8cd46b` + `e2dc950` + `3a78a5e`, alle gepusht). Geen opdracht vooraf; verzamelsessie, want er liep tussen 10 en 12 aug geen `/summary` — per de nummerregel (*per summary-ronde, niet per commit, ook na `/clear`*) is dat één sessie. **(1) `0dd0c64`**: #64 beschreef zijn diagnose maar niet zijn prioriteit, en dat leest een volgende sessie als werk; retries staan lokaal op 1, dus in normaal gebruik is het een groene run met label *flaky*. **(2) `8c0c455`**: `align-items: center` centreert de **doos** van de `<p>`, niet de tekst erin — zodra de copyright-regel afbreekt wordt de doos containerbreed en valt de tekst terug op `text-align: start`. Bug leefde in een smalle band (**≤385px fout, ≥390px goed**), wat verklaart waarom hij lang onopgemerkt bleef; gemeten −44,0/−122,5px afwijking op 375px, ná: 0/0. `text-wrap: balance` maakte 247/90px tot 171/166px. NEW `footer-copyright.spec.js`. **(3) `c8cd46b`**: `--color-cta-primary` droeg twee onverenigbare rollen — als CTA-achtergrond werkt merkgroen, als **tekst** haalde hij in light mode nergens AA. Over 12 pagina's tegen de *effectieve* achtergrond: **232 accent-tekstelementen, 101 onder AA → 0**. Nieuw token `--color-accent-text` op 22 declaraties; Green 900 op meting gekozen want de vanzelfsprekende Green 700 haalt AA níét op een `.section-band` (4,31:1). NEW `accent-text-contrast.spec.js`. **(4) `e2dc950`**: de vierde gids was toegevoegd in commits die alleen `gidsen.html` + `docs/products/*` raakten, dus elke aantal-claim daarbuiten bleef op 3 — `blog/welkom.html` beloofde "drie gidsen" pal naast de bundel-CTA die er vier levert. NEW **Check 13**, die N=4 en paginasom=72 **afleidt** uit `gidsen.html` i.p.v. hardcodeert. **(5) `3a78a5e`**: drie eerdere sessies meldden "6 posts promoten het Playbook twee keer" als redactionele keuze. Heisenberg vroeg de norm zélf te onderzoeken — *"misschien is die template wel niet correct"*. Dat was beslissend: `blog-template.md` **sprak zichzelf tegen** (mapping-tabel wijst `wmvpx` aan recon-posts toe, de regel eronder verbiedt dat), dus zes posts waren één fout document dat zich zes keer reproduceerde. Het echte defect was een ander en raakte **13 posts**: buiten de blog zei **8 van de 8** betaalde CTA's "Bekijk…", binnen de blog **13 van de 15** "Download…" — de blog was de enige plek die een download beloofde voor iets achter een betaalmuur. Nu 0/15. Geen prijzen in de copy (de site noemt nergens een bedrag; dat zou 15 verouderende plekken maken) — bij `wmvpx` doet "alle 6 fasen in ~19 pagina's" tegenover de 9 gratis het onderscheidende werk. NEW **Check 14**, **positief** geformuleerd omdat de eerste, negatieve versie de mutant `>Pak het Playbook<` liet overleven — en mijn scriptcommentaar bewéérde dat een andere assertie die afving. Punt 4 van het goedgekeurde plan (CTA-volgorde omdraaien in `leren-hacken.html`) **na meting geschrapt**: beide CTA's zitten contextueel goed. Bundel **1098,46 KB / 1120 — marge 1,9%**, de alarmgrens is in zicht.
...
- **Playwright:** **39 spec files, 296 `test()`-declaraties** per browser-project (Sessie 221: +2 specs / +6 declaraties — `footer-copyright.spec.js` en `accent-text-contrast.spec.js`. De twee nieuwe `validate-docs.sh`-checks (13 en 14) zijn **géén** Playwright-tests: ze bewaken statische tekst, dus ze horen in het shellscript dat al via de pre-commit hook draait — geen server, geen browser, geen suite van 48 minuten. Check 14 is met **7 mutanten, 7 rood** geverifieerd; de eerste, negatieve versie liet `>Pak het Playbook<` overleven, dus beide asserties zijn nu positief. Bundel-limiet 1120 KB, gemeten **1098,46 KB** — marge **21,54 KB (1,9%)**, was 24,46 KB (2,2%). De +2,92 KB komt volledig uit de twee CSS-commits (`8c0c455` + `c8cd46b`); de blog-CTA-wijziging kost +0,49 KB en telt **nul** in deze teller, want die meet `src/` + `styles/` + `index.html` en niet `blog/`. ⚠️ **Onder 2% marge raakt de volgende niet-triviale wijziging de grens** — 1000 → 1050 → 1100 → 1120 is drie bumps in 17 sessies, dus de vraag is dan niet weer een bump maar of dit nog het juiste getal is om te meten.) — *historische context Sessie 220:* nul nieuwe declaraties — deze sessie repareerde bestaande tests i.p.v. er bij te schrijven. Twee asserteerden niet wat ze beweerden: `performance.spec.js:480` nam 10/10 seriële runs een guard-tak die "er is niets gemeten" als geslaagd rapporteerde, en `responsive-breakpoints.spec.js:209` viel op een oorzaak die twee sessies lang verkeerd genoteerd stond. NEW: een goto-guard in `tests/e2e/fixtures.js` die alle 150 `goto`-aanroepen in 37 specs dekt en Netlify's bot-challenge herkent — geen test, wel de reden dat een volgende rode run leesbaar is. ⚠️ **Draai de suite tegen een lokale server**, niet tegen productie: drie parallelle motoren lokken de bot-protectie uit en dat gaf 5 van de 7 falers. `python3 scripts/nostore-server.py 8901 "$(pwd)" & BASE_URL=http://localhost:8901 npx playwright test`. Bundel-limiet 1120 KB, gemeten **1095,54 KB** — marge 24,46 KB (2,2%), was 26,10 KB. De 1,64 KB komt van de gidsen-link + zijn CSS. **De alarmgrens is nu in zicht: de volgende niet-triviale wijziging raakt hem, en dan is de vraag niet weer een bump** — 1000 → 1050 → 1100 → 1120 is drie bumps in 16 sessies. Die grens meet ongeminificeerde broncode sitebreed en is een driftalarm, geen perf-poort: Terminal Core (<400 KB minified, de echte poort) wordt met nul bytes geraakt.)

<!-- VALIDATE-BUNDLE-START Sessie 157 — ground-truth target voor scripts/validate-docs.sh --deep -->
<!-- src=722 styles=437 blog=474 assets=1737 (KB unminified, du -sb / 1024 basis; Sessie 221 ground-truth meting) -->
<!-- VALIDATE-BUNDLE-END -->
...
64. [ ] **`autocomplete-filesystem.spec.js:99` is flaky onder volle-suite-load — openstaande diagnose, GEEN baseline** (gevonden Sessie 220). Symptoom: `input.fill('cat /etc/pa')` + `Tab` laat de waarde ongewijzigd (verwacht `cat /etc/passwd`); viel als test 7 van 364 tijdens worker-opstart. **Wat gemeten is:** (a) **niet van Sessie 220** — A/B tegen `git show HEAD:tests/e2e/fixtures.js` met dezelfde opdracht gaf **1 rood op 4 runs aan beide kanten**, identieke verdeling; (b) reproduceert **niet** in isolatie: 0/10 mislukt in een kale run. **Twee hypotheses gefalsifieerd, niet doorgeven als waarheid:** (1) *"de app is nog niet gewired als de spec begint te typen"* — onjuist, `window.HackSimulator` bestond 8/8 al zodra `waitForSelector('#terminal-input')` terugkomt (`goto` wacht op `load`, dus main.js heeft gedraaid; resterende wachttijd 4-13 ms); (2) *"de legal-modal blokkeert Tab via de modal-guard/focus-trap"* — onjuist, 0/10 modal actief én 0/10 focus weg bij Tab. **Prioriteit: laag — géén eigen sessie waard.** De suite draait met `retries: 1` lokaal (`playwright.config.js:21`), dus in normaal gebruik rapporteert dit als een **groene run met het label `flaky`** en blokkeert het niets. Alle metingen in Sessie 220 zijn met `--retries=0` gedaan om schoon te kunnen tellen — dáár werd het pas een rode run. Pak dit op wanneer je tóch in de testsuite zit, niet als losse missie. **Wel bewust géén "bekende faler" noemen:** dat label is precies waar de staande regel van Sessie 217 tegen is geschreven — zodra iets zo heet, leest niemand de foutmelding nog. **Volgende stap wanneer je eraan toekomt:** reproduceren ónder load (parallelle workers) i.p.v. in isolatie, en `focus-trap.js:74` + `input.js:138` instrumenteren op het faalmoment. Zolang de oorzaak niet gemeten is, is dit een openstaande diagnose — zie de staande regel in `.claude/CLAUDE.md` (Sessie 217): een faler zonder gemeten oorzaak is geen bekende faler
...
**Versie:** 5.93 (Sessie 219 — **Onder "in cijfers" was de homepage één blok, en de band die als voorbeeld gold maakte in light mode nul verschil.** Nulmeting: 2390px @1440 en 3078px @375 zonder achtergrondwissel = 3,8 schermen, tegen 1,6 boven de vouw. Twee bugs die alleen uit een meting komen: `rgba(248,248,248,0.8)` over `#f8f8f8` composit naar exact de paginakleur (Δ0, de band was onzichtbaar), en de kaarten zijn `rgba(22,27,34,α)`-tinten van de nieuwsbriefkleur — dus een lichtere band lost ze op (Δ0). Daar liep ik zelf in met mijn eerste prototype. Opgelost met één regel i.p.v. drie ad-hoc waarden: pagina = oppervlak, band = verdieping, kaart = verhoging (`--color-bg-alt` + `.section-band` + een full-bleed-modifier met `padding-inline: max()` die een ontbrekende inner wrapper vervangt). Ná: 949px / 913px = 1,05 / 1,12 scherm. Δ band-vs-pagina 2 → 8 (dark) en 0 → 12 (light); kaart-op-band 3 → 6 / 19. Vier CSS-regels weg, twee erbij, en `.results-grid` ligt eindelijk op dezelfde rail. Twee cascade-verliezen die alleen `getComputedStyle` verraadt (gelijke specificiteit + latere bronvolgorde; een redundante regel die op (0,2,0) won). 26/26 groen over drie motoren; de mutant reproduceert de bug cijfer voor cijfer (2392px / 3070px). Kost 2,88 KB terwijl de diff nétto regels verwijdert — het commentaar is langer dan de code. De juridische sample bewust NIET aan de homepage toegevoegd. Bundel 1093,90 KB / 1120 (marge 2,3%). Volledig: `docs/sessions/current.md`)

**Versie:** 5.92 (Sessie 218 — **De strook onder de terminal was AdSense-vulling die AdSense vijf maanden overleefde.** Commit `1cc04ff` heet letterlijk "full-viewport terminal + scroll hint for AdSense content"; advertenties gingen eruit in Sessie 208, de strook bleef. Nulmeting: 2424px = 65% van de pagina, 396 woorden, 3 links — en zes command-kaarten die `nmap`/`hashcat`/`sqlmap` bij naam noemen zonder link te zijn. Ná: 1784px (−26%), 12 links, en de pagina heeft voor het eerst een `<h1>`. "Zo begin je" geschrapt omdat de terminal het zélf zegt op het juiste moment; de rol-rangschikking volgt de gedocumenteerde north-star (activation, niet lezen). Drie pre-existing bugs meegenomen: verlopen bloglabels, de hele strook onzichtbaar zonder JS, en `WebPage.name` ≠ `<title>` ≠ zichtbare kop. Eén eigen bug door te meten gevangen (CTA 193×22px, onder de 44px-grens). NEW `edu_section_reached`: de doorscroll-rate was er in vijf maanden nooit. 7 mutanten tweezijdig bewezen, overlever verklaard. Kost ~3,3 KB in plaats van te besparen — de winst is hoogte, links en meetbaarheid. Bundel 1091,02 KB / 1120. Volledig: `docs/sessions/current.md`)

**Versie:** 5.91 (Sessie 217 — **Vier vastgelegde pre-existing punten opgeruimd; drie van de vier metingen die erbij stonden klopten niet.** De 76px-strook is zichtbaar (16,39:1 in light) en raakt 10 pagina's i.p.v. 9 — opgelost door de reserve naar de dónkere footer te verhuizen, met de onvoorwaardelijkheid gemeten (9481/9481/9481 over drie toggles) i.p.v. aangenomen. Het badge-contrast is 2,85:1 tegen de eigen achtergrond en niet de genoteerde 3,10:1 tegen de pagina — dus onder AA, en de notitie was te gunstig; opgelost met een `--eyebrow-text`-token (17,12:1 in light, merkgroen in dark). De terminal-overflow was al gefixt op 07 jul door commit `3d7df13` en bleef vijf sessies open staan omdat er geen assertie was die dat kon melden; de test is alsnog toegevoegd en reproduceert de oude notitie cijfer voor cijfer op de mutant. Budget expliciet naar 1120 KB (2,94% marge). 3 mutanten tweezijdig bewezen, inclusief de overlevers nagelopen. Bundel 1087,05 KB / 1120. Volledig: `docs/sessions/current.md`)

**Versie:** 5.90 (Sessie 216 — **De mobiele CTA-balk verscheen waar hij niets toevoegde, en de guard die dat moest bewaken scrollde nooit.** De balk stapt nu opzij zodra het middelpunt van een primaire CTA vrij ligt — de enige grens die "altijd een tikbare CTA" én "nooit twee identieke" per constructie waarmaakt; 884-893 posities à 10px in drie engines gaven nul gaten en nul dubbels. IntersectionObserver boven `position: sticky` (die lost de plaats op, niet het mechanisme), met IO als trigger en één geometrisch predicaat als regel — blijft daardoor correct bij toestelrotatie. De conversie-guard scrollde nooit (`scroll-behavior: smooth` + synchrone lus → `scrollY` bleef 2px) en mat bounding boxes i.p.v. aantikbaarheid; op één mutant is de oude 3× groen en de nieuwe 3× rood. Eén zelf-geïntroduceerde bug zelf gevangen (`transition: all` liet de knop ~150ms achterlopen op zijn balk). 5 nieuwe asserties, elk met de mutant bewezen. Bundel 1084,92 KB / 1100. Volledig: `docs/sessions/current.md`)

...
**Status:** M7 Gamification ✅ 100% | M6 Tutorial System 88% | M5.5 Monetization ✅ Live + Brevo deliverability tuned + Gumroad v1.0 + Lead magnet (LIVE on hacksimulator.nl) | Doc-protocol refactor + forcing function (Sessie 140) | Terminal Core ⚠️ ~547 KB minified + ⚠️ **Lighthouse Mobile 39-40 / Desktop 64-69** gemeten (Sessie 142+143, hoofdoorzaak AdSense ecosysteem 251.7 KB / 73% blocking-time)
**Sprint:** Sessie 143: Third-party audit voltooid (item #25 ✅) — frame-bias-correctie bevestigd via Lighthouse JSON-parse: GA4/Brevo/Ko-fi NIET op terminal.html, AdSense + DoubleClick = 65% transfer + 788 ms blocking, ad-slot-module 77% ongebruikt op deze pagina; 3 paden voor #24-heropening klaar in docs/perf-third-party-audit.md
...
**Bundle (geverifieerd 28 mei 2026, Sessies 140-143):**
- **Site totaal:** ~1966 KB unminified | src/ 627 KB | styles/ 268 KB | blog/ 369 KB (12 files: 10 posts + index + welkom) | assets/ 702 KB | HTML ~155 KB
...
- **Lighthouse on-wire (Sessie 142 baseline + Sessie 143 reproduction, productie /terminal.html):** **Mobile 39-40/100, Desktop 64-69/100** | Total transferred 624 KB / 118 requests | first-party scripts ~98 KB gzipped / ~93 files | **third-party 353 KB / 12 requests / 6 origins (Sessie 143 ground truth):** AdSense `pagead2.googlesyndication.com` 230.5 KB / 788 ms blocking | Google Fonts `fonts.gstatic.com` 99.8 KB / 0 ms blocking (al optimaal) | `ep1+ep2.adtrafficquality.google` 21.2 KB / 97 ms blocking | `fonts.googleapis.com` 1.5 KB. **GA4 + Brevo + Ko-fi NIET geladen op terminal.html** (consent-default-denied → tracker.js skipt gtag.js correct; Brevo/Ko-fi alleen op index/sample-pentest). **Frame-bias correctie compleet:** AdSense ecosysteem = 65% transfer + 73% blocking-time, dominante bottleneck. AdSense ad-slot script 77% ongebruikt op terminal.html (geen `<ins>` slots in body). Zie `docs/perf-third-party-audit.md`
...
24. [ ] **Bundle-optimalisatie sprint** (uit Sessie 141 meet-resultaat, ⏸️ PAUSED Sessie 142, scope verfijnd Sessie 143) — Terminal Core ~547 KB minified zit ~37% boven 400 KB budget. **Sessie 143 audit (`docs/perf-third-party-audit.md`) onthult dat eigen-code bundle NIET de hoofdoorzaak is van Lighthouse Mobile 40 / Desktop 69**; AdSense ecosysteem domineert (788 ms blocking mobile, 132.9 KB ongebruikte AdSense-code). Drie paden klaar voor heropening: **Pad C1** (low-risk quick wins ~30 min: preconnect pagead2 + animations.css deferred + fetchpriority hints = ~275 ms LCP) / **Pad C2** (Heisenberg verifieert AdSense Auto-ads dashboard-state → indien UIT, adsbygoogle.js verwijderen van terminal.html = ~230 KB / ~788 ms gratis besparing) / **Pad C3** (status-quo + budget-herijking 624 KB on-wire als baseline). Beslissing-keuze vereist vóór sprint-start.
...
26. [ ] **`box-utils.js` bootup-time profile** (out-of-scope-vondst Sessie 143, defer tot ná #24-sprint) — Lighthouse `audits["bootup-time"]` op `/terminal.html` mobile rapporteerde `src/utils/box-utils.js` op **309 ms total / 200 ms scripting** = #3 op de bootup-tabel direct na AdSense ad-slot (915 ms) en adsbygoogle.js (409 ms). Verrassend hoog voor utility-code (vergelijk: main.css 102 ms / 0 ms scripting). Hypotheses te onderzoeken: (a) `measureCharWidth()` met `getComputedStyle` + Canvas API draait herhaaldelijk zonder cache-hit (regels 12-50), (b) cache-key `fontSize|fontFamily` faalt bij `font-display:swap` font-substitutie, (c) box-render-loops over groot output-volume. Scope: ~30 min profile (Chrome DevTools Performance tab) + cache-warming patch indien (a)/(b) bevestigd. Defense-in-depth persistence: dit task + inline TODO bovenaan `src/utils/box-utils.js` regel 1.
...
**Laatst bijgewerkt:** 28 mei 2026 (Sessie 143)
**Versie:** 4.7 (Sessie 143 — Third-party audit voltooid, item #25 ✅ gesloten. Lighthouse JSON-parse op /terminal.html onthulde frame-bias-correctie binnen Sessie 142's correctie: GA4/Brevo/Ko-fi NIET op terminal.html, AdSense ecosysteem (pagead2 230.5 KB + ep1/ep2.adtrafficquality 21.2 KB) = 73% blocking-time + 65% transfer. Smoking gun: AdSense ad-slot script 132.9 KB / 77% ongebruikt (0 `<ins>` slots in body). Reproduction Mobile 39→40, Desktop 64→69 binnen run-noise. Output `docs/perf-third-party-audit.md` met 3 paden C1 (quick wins ~275 ms) / C2 (Heisenberg verifieert AdSense Auto-ads dashboard, indien UIT ~230 KB + ~788 ms besparing) / C3 (budget-herijking). Item #24 scope verfijnd met deze pad-keuze.)
...
**Bundle (geverifieerd 28 mei 2026, Sessies 140-143):**
- Site totaal **~1966 KB unminified** | src/ 627 KB + styles/ 268 KB + HTML ~155 KB + blog/ 369 KB + assets/ 702 KB
- **Terminal Core (runtime van terminal.html, gemeten Sessie 141):** **~781 KB unminified** (~547 KB minified geschat) | HTML 19 KB + CSS 160 KB (6 files) + JS module-graph 601 KB (99 files reachable van entry points). ⚠️ ~37% boven 400 KB budget — beslissing in item #24 op pauze; Pad C-keuze uit Sessie 143 audit
- **Lighthouse on-wire (Sessie 142 + 143):** Mobile 39-40, Desktop 64-69 (reproduceerbaar binnen run-noise). Total transferred 624 KB / 118 req. First-party scripts ~98 KB gzipped vs **third-party 353 KB / 12 req / 6 origins**. AdSense (pagead2 + DoubleClick + adtrafficquality) = 73% blocking-time + 65% transfer. TBT 3,020-3,270 ms mobile / 460-610 ms desktop. Smoking gun: AdSense ad-slot-module 132.9 KB (77%) ongebruikt op terminal.html. Zie `docs/perf-third-party-audit.md`.

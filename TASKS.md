...
**Laatst bijgewerkt:** 02 aug 2026 (Sessie 207; audit oude follow-up-lijstjes + 4 werkpakketten)
**Sprint:** Sessie 207: **Audit van drie oude "optionele follow-up"-lijstjes → 3 items gedaan, 4 verworpen, 4 werkpakketten uitgevoerd.** Heisenberg vroeg wat er van drie eerder genoteerde ideeënlijstjes (leerpad-features, blog/SEO, border-refactor) al gedaan was, met de opdracht **alleen bezoekerswaarde** aan te pakken en de rest te schrappen. Uitkomst van de audit: **4 items al klaar** (Google Search Console DNS-geverifieerd sinds Sessie 160 + sitemap in robots.txt; RSS-feed op `/feed.xml` met 14/14 items, bewaakt door `validate-docs.sh:475-524`; related-posts "Verder lezen" op 14/14 posts = 56 interne links; GA4 + Consent Mode v2 over 943 regels/8 bestanden). **4 verworpen met reden** (leerpad-% in `dashboard` = derde weergave van data die `leerpad` én `dashboard` al tonen; phase-badges = viering bestáát al en `next` wordt ná élk geleerd command aangeboden, dus geen gat; leerpad-certificaat = certificaten bestaan al voor challenges én tutorials, en "zet dit op je CV" over een browser-simulator is overbelofte; social sharing = tone-conflict; SSG = PRD §13-conflict en de validators lossen de duplicatie al op; blog-engagement-events = GA4 meet scroll/engagement standaard; RSS-link in root-heads = winst ≈ 0). **Uitgevoerd: W4** latente CSS-bug — `.landing-nav-wrapper` (`landing.css:78`) en `.landing-footer` (`main.css:1025`) hingen aan `--color-border` (het *content*-token met 90 call-sites) i.p.v. `--color-border-dark-frame`; werkte alleen doordat beide in dark mode toevallig `#30363d` zijn. Rood-op-mutant bewees de koppeling vóór (navbar/footer werden rood) en ná (niet meer); 8/8 metingen `rgb(48,54,61)` op index + terminal, dus visueel een no-op. Twee redundante light-overrides geschrapt, géén `?v=`-bump (identieke uitkomst = cache busten voor niets). Style-guide-sectie `.category-btn` bleek volledig gedrift (**5** niet-bestaande variabelen + elke state-waarde fout) → verbatim herschreven uit `blog.css:115-137`; focus-management gecorrigeerd naar de globale `:focus-visible`-regel; `#d0d0d0`-borderclaim → `#e0e0e0`. **W1** FASE 4 (Security Tools) had als enige fase géén vieringsmoment — vijfde transitie toegevoegd in `next.js` (`phase4Commands` bestond al op regel 25). Desktop 15 regels allemaal exact 104 tekens, mobiel borderless, tweede aanroep herhaalt niet. **W3** tutorial-certificaten konden alleen naar het klembord terwijl challenge-certificaten wél downloadbaar waren — `downloadCertificate()` toegevoegd (ASCII-conversie nu gedeeld door copy én download), `tutorial cert download`, man-page + usage bij. Gemeten: `hacksimulator-tutorial-recon.txt`, 2204 tekens, **nul** niet-ASCII-tekens. **W2** alle 15 blog-bestanden deelden één generieke og:image → `scripts/build-blog-og-images.mjs` (resvg, model = `build-product-covers.mjs`) genereert 14 kaarten die titel + categorie **uit de post zelf** lezen, dus géén 8e lockstep-locatie. Exact 1200×630 (niet @2x, anders liegen de bestaande width/height-tags), 14/14 nageteld, 42 tags vervangen, 548 KB in de budgetloze SEO-pijler. Eerlijk over het plafond: Google gebruikt og:image niet in zoekresultaten — de winst zit in gedeelde links, en vooral in het feit dat elke volgende post zijn kaart nu gratis erft. **Bijvangst:** de Versie-drift TASKS.md 5.79 ↔ CLAUDE.md 5.80 stamt uit Sessie 206 en glipte langs de pre-commit-hook omdat Check 7 alleen in `--deep` draait.
...
<!-- src=688 styles=396 blog=456 assets=1555 (KB unminified, du -sb / 1024 basis; Sessie 207 ground-truth meting — assets +525 KB door 14 per-post og:images in assets/blog/ (SEO/content-pijler, budgetloos; runtime terminal.html ongemoeid); src +3 KB door FASE 4-transitie + tutorial-cert-download) -->
<!-- VALIDATE-BUNDLE-END -->

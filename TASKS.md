...
**Sprint:** Sessie 145: Item #26 ✅ gesloten via Frame B (Lighthouse-attributie-bias). Verify-first-plan uitgevoerd met 3-run Lighthouse@11 + raw trace.json parsing + Playwright cold/warm-meting (5-iteratie cachebust). Multi-metric bewijs: raw trace box-utils 1.3 ms vs Lighthouse-attributie 230 ms = factor 177x mismatch. Hypotheses (a)/(b)/(c) uit Sessie 143-formulering allemaal gefalsifieerd door data — geen code-actie. Echte mobile-score-bottleneck verschoven naar Style/Layout (2172 ms in `mainthread-work-breakdown`). Kandidaat-spawn: item #28 Layout/Style perf-audit. Verify-first-plan + multi-metric tabel in `docs/perf-third-party-audit.md` §2 + plan-file `/home/willem/.claude/plans/heisenberg-hier-pak-item-pure-cascade.md`.
...
28. [ ] **Style/Layout perf-audit op terminal.html** (Sessie 145 spawn uit item #26 Frame B closure) — Trace-bewijs uit Sessie 145 (`/tmp/perf-item26/lh-run2-0.trace.json` + `mainthread-work-breakdown` audit): **styleLayout 2172 ms** = écht dominante cost-driver op mobile, **5.8x de hele scripting-totaal** (376 ms verdeeld over alle scripts). Top single tasks in boot-window (0-5 s): `Layout` 195 ms + 137 ms + 87 ms (gezamenlijk 419 ms = 19% van totale styleLayout). Mobile-score-verbetering richting plan-target 70-80 moet hier vandaan komen. **Audit-vragen:** (a) Welke DOM-injecties tijdens boot triggeren grote Layouts? Kandidaten: navbar (`navbar-terminal.js`), footer-injection (`init-components.js`), legal-modal (`ui/legal.js` — zag al 53.9 ms FunctionCall in Sessie 145 trace), onboarding-welkom-typewriter. (b) Zijn er CSS-rules met layout-triggering properties (width/height/margin/padding op vaak-reflowed elementen) die naar `transform`/`opacity` (composite-only) verplaatst kunnen worden? (c) Gebruikt main.css `:has()` of complexe selectors die per DOM-mutatie hele subtrees forceren te recomputen? (d) Welke aandeel van Layout-tijd komt door 3rd-party Google Fonts CLS (Inter/JetBrains Mono/Space Grotesk swap)? **Scope:** ~2-3 uur — capture Chrome DevTools Performance trace via headless Lighthouse `--save-assets`, parse `LayoutShift` + `Layout` events met causal-attribute via `frame` field, identificeer top-5 DOM-mutaties die boot-Layout dominant maken. Verwachte impact: mobile +5-15 score indien top-3 DOM-injecties uitgesteld of layout-stabiel gemaakt kunnen worden. Multi-metric vereiste analoog #26: 3-run mediaan Lighthouse + Playwright `PerformanceObserver({type: 'layout-shift'})` bewijs. Defense-in-depth: dit task + inline comment in main.css of init-components.js bij gevonden hotspot. Plan-file conditional na trace-data binnen.
...
**Laatst bijgewerkt:** 29 mei 2026 (Sessie 145)
**Versie:** 4.9 (Sessie 145 — Item #26 ✅ gesloten via Frame B (Lighthouse-attributie-bias). Verify-first-plan uitgevoerd met 3-run Lighthouse@11 mobile met `--save-assets` + Python parse-script over raw `trace.json` events + Playwright cold/warm-meting via 5-iteratie cachebust dynamic-import. Multi-metric bewijs: raw trace box-utils.js totaal 1.3 ms X-phase dur (parse op worker-thread); Playwright coldCallMs 1.4 ms (Frame A drempel >20 ms gefalsifieerd) + warmCallMs 0.1 ms (hypothese (b) cache-failure gefalsifieerd); Lighthouse `audits["bootup-time"]` rapporteert 230 ms scripting = factor ~177x mismatch. Hypotheses (a)/(b)/(c) uit Sessie 143-formulering allemaal gefalsifieerd door data. Echte cost-drivers uit `mainthread-work-breakdown`: Style/Layout 2172 ms + scriptEvaluation totaal 376 ms + parseHTML 115 ms. Top single tasks = Layout 195/137/87 ms (2x meer dan alle scripting). Frame B-uitkomst zonder code-wijziging is legitieme vervulling plan §3. Plan-file: `/home/willem/.claude/plans/heisenberg-hier-pak-item-pure-cascade.md`. Multi-metric tabel in `docs/perf-third-party-audit.md` §2. Direct learning: `bootup-time` per-URL-cijfers vereisen multi-metric verificatie vóór gebruikt als beslis-anchor. Kandidaat-spawn item #28 voor Layout/Style perf-audit indien Heisenberg akkoord.)

...
**Laatst bijgewerkt:** 07 aug 2026 (Sessie 213; gidsen-grid + CTA-uitlijning + site-brede navbar-inklapband)
...
**Current Status:** ✅ LIVE — Playwright E2E: **32 spec files / 243 `test()`-declaraties** (Chromium, Firefox, WebKit). Let op: het aantal *gedraaide* tests ligt hoger dan het aantal declaraties sinds Sessie 212 — `lead-magnet.spec.js` en (sinds Sessie 213) `navbar-collapse.spec.js` genereren zijn scenario's in een `for…of` over beide samples, dus 1 declaratie = 2 tests. Die suite alleen: **60 passed / 0 failed tegen productie** (6 skips vallen weg zodra je tegen productie draait i.p.v. lokaal, want `_headers` is Netlify-only). | Ko-fi + Brevo (deliverability getuned) + Gumroad v1.0 + **2 lead magnets met elk een eigen Brevo-formulier + automation (Sessie 212)** | **advertenties volledig verwijderd (Sessie 208)**
...
<!-- src=712 styles=419 blog=474 assets=1737 (KB unminified, du -sb / 1024 basis; Sessie 214 ground-truth meting) -->
<!-- VALIDATE-BUNDLE-END -->

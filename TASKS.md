...
**Laatst bijgewerkt:** 09 aug 2026 (Sessie 218; AdSense-vulling onder de terminal gehalveerd)
...
**Current Status:** ✅ LIVE — Playwright E2E: **37 spec files / 288 `test()`-declaraties** (Chromium, Firefox, WebKit). Let op: het aantal *gedraaide* tests ligt hoger dan het aantal declaraties sinds Sessie 212 — `lead-magnet.spec.js`, `navbar-collapse.spec.js` (Sessie 213) en sinds Sessie 215 drie blokken in `hero-demo.spec.js` genereren hun scenario's in een `for…of`, dus 1 declaratie = 2-3 tests. Sessie 218-verificatie: **145 passed / 2 failed** over de vijf specs die de gewijzigde bestanden raken, drie engines; beide falers zijn `responsive-breakpoints.spec.js:209` en aantoonbaar contentie, geen regressie (32/32 groen serieel, oud én nieuw). | Ko-fi + Brevo (deliverability getuned) + Gumroad v1.0 + **2 lead magnets met elk een eigen Brevo-formulier + automation (Sessie 212)** | **advertenties volledig verwijderd (Sessie 208)**

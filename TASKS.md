...
**Laatst bijgewerkt:** 18 aug 2026 (Sessie 226; blog-analyse → tapdoelen, AAA-contrast, inhoudsopgave over 15 posts, drie nieuwe guards)
...
- **E2E-bundle-test-limiet (performance.spec.js): 1120 KB** — gemeten stand **1118,63 KB** (Sessie 226, marge **1,37 KB = 0,1%**). ⚠️⚠️ **De volgende niet-triviale wijziging breekt de poort.** De +12,17 KB is volledig van deze sessie en attribueert exact: `blog.css` +4,3 KB, `src/ui/blog-toc.js` +5,4 KB (nieuw), `src/ui/blog-filter.js` +2,3 KB (nieuw), `main.css` +0,5 KB. **Maar de vraag is nu niet "bump" en ook niet "knip bytes" — het is of deze formule het juiste meet.** Hij telt `styles/**/*.css` en `src/**/*.js`, dus `blog.css` (43 KB) plus de twee nieuwe blogmodules tellen mee, terwijl `terminal.html` ze **nooit** laadt en `.claude/CLAUDE.md` de blog expliciet "SEO/content-pijler **budgetloos**" noemt. Doc en gate spreken elkaar dus tegen, en deze sessie maakte dat zichtbaar door er als eerste blogcode in te schrijven. Drie opties, alle drie een beslissing van Heisenberg: (a) blog uitsluiten uit de formule zodat hij meet wat hij beweert te meten, (b) een aparte blogteller ernaast, (c) bumpen — maar dat is dan de **vierde** bump in 22 sessies (1000 → 1050 → 1100 → 1120) en dat patroon zegt dat het getal het probleem niet is. *Voorheen (Sessie 225): 1106,46 KB, marge 13,54 KB = 1,21%.* ⚠️ **De +1,85 KB is níét van deze sessie:** alle vijf de nieuwsbrief-commits raakten uitsluitend `docs/newsletter/`, wat buiten deze formule valt. De delta komt volledig uit `styles/blog.css` in **`d2d2484`** — een commit die ná de Sessie 224-summary viel en daardoor ongelogd bleef. Attribueer aan de veroorzaker, niet aan de laatste commit (Sessie 221-les). *Voorheen (Sessie 224): 1104,61 KB, marge 15,39 KB = 1,37%; de +1,18 KB was het CSS-commentaar bij de `overflow-wrap`-fix, dat de rekensom én de afgevallen `hyphens`-meting vastlegt — de regel zelf is 4 regels.* Deze regel stond tot Sessie 222 op "1050 → 1100 (Sessie 214)" terwijl de constante al sinds Sessie 217 op 1120 stond: **de notitie liep achter op de code**. Geschiedenis: 1000 → 1050 (Sessie 204) → 1100 (Sessie 214) → 1120 (Sessie 217) = **drie bumps in 14 sessies**, dus de volgende vraag is niet "bump". Eerder: Eerder 1000 → 1050 (Sessie 204, jul 2026) na organische groei door funnel/analytics-werk Sessie 190-203. Aanleiding nu: de interactieve hero-terminal (`src/ui/hero-repl.js` + hero-CSS + markup) kostte 24 KB tegen 4,78 KB resterende ruimte; gemeten ná de wijziging **1069,20 KB**. De test meet unminified source (src/ JS + styles/ CSS + src/ui CSS + index.html) en is dus een **driftalarm**, geen perf-poort: de echte poort blijft Terminal Core <400 KB minified, en die wordt met nul bytes geraakt — `hero-repl.js` zit alleen in de module-graph van `index.html`, niet in die van `terminal.html` (zie regel hieronder + #24). Marge bewust ~31 KB (2,9%): een limiet die vlak boven de huidige stand ligt vuurt op élke wijziging en wordt dan weggeklikt in plaats van onderzocht. **Stand Sessie 215: 1078,05 KB — marge 20,95 KB (2,0%).** De +8,8 KB is grotendeels commentaar (landing.css +6,3 KB); de prosa is al uitgedund met behoud van elke meting en valkuil. De volgende wijziging tikt de alarmgrens aan: dan is de vraag niet "hoe knip ik bytes" maar of 1100 nog klopt.
...
- **Playwright:** **42 spec files, 312 `test()`-declaraties** per browser-project — **Sessie 226: +1 spec / +7 declaraties** (`blog-navigation.spec.js`). Gemeten, niet uitgerekend: het bestand heeft 7 `test()`-regels waarvan de laatste in een `for…of` over beide thema's zit, dus 7 declaraties → 8 gedraaide tests. Dekt drie dingen die alleen **gerenderd** meetbaar zijn en dus niet in `validate-blogs.sh` kunnen wonen: tapdoelmaat (de bug zat in `display`, niet in een CSS-waarde die je kunt greppen), contrast tegen de **effectieve** achtergrond in beide thema's, en het TOC-gedrag (deeplink onder de navbar, actieve sectie volgt de scroll). Elke assertie heeft een tak die faalt bij nul treffers. Verificatie: 186 passed / 0 failed over drie motoren, samen met `blog-meta-separators`, `blog-theme-toggle` en `accent-text-contrast` (die laatste is de token-gevoelige suite en dekt de `--color-text-dim`-wijziging af). — *voorheen (Sessie 225): +0 / +0.* Deze sessie raakte nul testbestanden; de +1/+1 t.o.v. de vorige notitie komt uit **`d2d2484`** (`blog-meta-separators.spec.js`, 212 regels), een commit die ná de Sessie 224-summary viel en daardoor ongelogd bleef. Bij het meten viel op dat de docs 40/304 claimden terwijl de boom er 41/305 had — de teller liep dus al achter vóór deze sessie begon. — *voorheen (Sessie 224): +1 spec / +1 declaratie* (`legal-pages-overflow.spec.js`, die uit die ene declaratie **9 tests** genereert = 27 over drie motoren). Eerste assertie ooit op `assets/legal/*`. Twee asserties met verschillende faalverzamelingen: A meet documentoverflow, B meet of koptekst binnen de eigen box blijft — @414px is A groen (397 < 414) terwijl B nog 3px overschrijding ziet. Geverifieerd met 2 mutanten die elkaars complement zijn: fix eruit → 9 rood, allemaal terms (A @320/@375, B óók @414); `min-width:700px` op de body → 27 rood op A, B overal groen. Die tweede is het enige bewijs dat privacy/cookies werkelijk gemeten worden — die zijn vóór én ná de fix groen. — *voorheen (Sessie 223): 39 / 303, ongewijzigd*: die sessie voegde twee guards toe (`validate-docs.sh` Check 16 en een AI-melding-assertie in Check 7 van `validate-blogs.sh`), maar dat zijn géén Playwright-tests. Beide bewaken statische tekst, dus ze horen in de shellscripts die al via pre-commit én CI draaien — geen server, geen browser. (Sessie 221: +2 specs / +6 declaraties — `footer-copyright.spec.js` en `accent-text-contrast.spec.js`. De twee nieuwe `validate-docs.sh`-checks (13 en 14) zijn **géén** Playwright-tests: ze bewaken statische tekst, dus ze horen in het shellscript dat al via de pre-commit hook draait — geen server, geen browser, geen suite van 48 minuten. Check 14 is met **7 mutanten, 7 rood** geverifieerd; de eerste, negatieve versie liet `>Pak het Playbook<` overleven, dus beide asserties zijn nu positief. Bundel-limiet 1120 KB, gemeten **1098,46 KB** — marge **21,54 KB (1,9%)**, was 24,46 KB (2,2%). De +2,92 KB komt volledig uit de twee CSS-commits (`8c0c455` + `c8cd46b`); de blog-CTA-wijziging kost +0,49 KB en telt **nul** in deze teller, want die meet `src/` + `styles/` + `index.html` en niet `blog/`. ⚠️ **Onder 2% marge raakt de volgende niet-triviale wijziging de grens** — 1000 → 1050 → 1100 → 1120 is drie bumps in 17 sessies, dus de vraag is dan niet weer een bump maar of dit nog het juiste getal is om te meten.) — *historische context Sessie 220:* nul nieuwe declaraties — deze sessie repareerde bestaande tests i.p.v. er bij te schrijven. Twee asserteerden niet wat ze beweerden: `performance.spec.js:480` nam 10/10 seriële runs een guard-tak die "er is niets gemeten" als geslaagd rapporteerde, en `responsive-breakpoints.spec.js:209` viel op een oorzaak die twee sessies lang verkeerd genoteerd stond. NEW: een goto-guard in `tests/e2e/fixtures.js` die alle 150 `goto`-aanroepen in 37 specs dekt en Netlify's bot-challenge herkent — geen test, wel de reden dat een volgende rode run leesbaar is. ⚠️ **Draai de suite tegen een lokale server**, niet tegen productie: drie parallelle motoren lokken de bot-protectie uit en dat gaf 5 van de 7 falers. `python3 scripts/nostore-server.py 8901 "$(pwd)" & BASE_URL=http://localhost:8901 npx playwright test`. Bundel-limiet 1120 KB, gemeten **1095,54 KB** — marge 24,46 KB (2,2%), was 26,10 KB. De 1,64 KB komt van de gidsen-link + zijn CSS. **De alarmgrens is nu in zicht: de volgende niet-triviale wijziging raakt hem, en dan is de vraag niet weer een bump** — 1000 → 1050 → 1100 → 1120 is drie bumps in 16 sessies. Die grens meet ongeminificeerde broncode sitebreed en is een driftalarm, geen perf-poort: Terminal Core (<400 KB minified, de echte poort) wordt met nul bytes geraakt.)

<!-- VALIDATE-BUNDLE-START Sessie 157 — ground-truth target voor scripts/validate-docs.sh --deep -->
<!-- src=733 styles=447 blog=494 assets=1737 (KB unminified, du -sb / 1024 basis; Sessie 226 ground-truth meting) -->
<!-- VALIDATE-BUNDLE-END -->
...
68. [ ] **`privacy.html` en `cookies.html` voeren Engelse koppen op een `lang="nl"`-pagina (Sessie 224)** — `<h1>`, `<title>`, `og:title` en `twitter:title` zeggen **"Privacy policy"** en **"Cookie policy"**, terwijl de rest van de site er in het Nederlands naar verwijst: *"Lees ons **privacybeleid**"* (`index.html:828,951`, `sample-pentest.html:184`, `sample-juridisch.html:184`). De pagina heet dus anders dan waarnaar gelinkt wordt. Strijdig met de NL-UI-regel (PRD §6.6) en met de zinskapitaal-conventie. **Niet meegenomen in Sessie 224** omdat een hernoeming 4 metatags × 2 bestanden, `sitemap.xml` en de linklabels op ~20 pagina's raakt — eigen taak met SEO-lockstep, geen bijvangst van een CSS-fix. Gemeten en géén blokkade: `Privacybeleid` en `Cookiebeleid` passen @320px met **0 overflow, ook zónder** de fix van #67 (ze zijn korter dan de 280px contentbox). De aanname in het Sessie-224-plan dat de fix die hernoeming "ongevaarlijk maakt" was dus te sterk — hij was al veilig; #67 is insurance voor iets langers.
...
70. [ ] **De bundelformule telt blog-assets tegen een "Terminal Core"-budget (Sessie 226)** — stand **1118,63 / 1120 KB, marge 0,1%**: de volgende niet-triviale wijziging breekt de poort. `performance.spec.js:132-141` sommeert `src/**/*.js` + `styles/**/*.css` + `src/ui/**/*.css` + `index.html`, dus `blog.css` (43 KB) en de twee nieuwe blogmodules tellen mee — terwijl `terminal.html` ze **nooit** laadt en `.claude/CLAUDE.md` de blog expliciet "SEO/content-pijler **budgetloos**" noemt. Doc en gate spreken elkaar tegen; deze sessie maakte dat zichtbaar door er als eerste blogcode in te schrijven. Drie opties: (a) blog uitsluiten zodat de formule meet wat hij beweert, (b) aparte blogteller, (c) bumpen — maar dat is de **vierde** bump in 22 sessies (1000 → 1050 → 1100 → 1120), en dat patroon zegt dat het getal het probleem niet is. **Beslissing van Heisenberg, niet van de volgende sessie die toevallig tegen de grens loopt.**
71. [ ] **`--color-link` haalt AAA niet in light mode (Sessie 226)** — `#0969da` op wit meet **5,19:1**; AA gehaald, AAA (7,0) niet. Raakt **élke** link op de site, niet alleen de blog. Bewust buiten scope gehouden: een blogbevinding hoort niet de sitebrede linkkleur te herzien, en de blast radius vraagt een eigen meting over alle paginatypes. Twee kleinere restpunten uit dezelfde meting: `--color-text-dim` haalt op `--color-bg-hover` (#21262d) **6,34** — genoteerd in `main.css`, en de `.related-category`-badge is opgelost met een eigen token naar het `--eyebrow-text`-precedent (Sessie 217) in plaats van via `--color-link`.

...
**Laatst bijgewerkt:** 09 aug 2026 (Sessie 219; sectieritme homepage + band-token)
...
- **Playwright:** 37 spec files, 290 `test()`-declaraties per browser-project (Sessie 219: +2 in `homepage-conversion.spec.js` — een geparametriseerd sectieritme-blok over 2 viewports × 2 thema's plus een content-rail-assertie, samen 5 gedraaide tests per engine; de eerste asserties ooit op de visuele hiërarchie van de homepage. Bundel-limiet 1120 KB, gemeten **1093,90 KB** — marge 26,10 KB (2,3%), was 28,98 KB. ⚠️ Sessie 219 kostte 2,88 KB terwijl de diff nétto CSS-regels verwijdert: het commentaar dat de light-mode-Δ0-bug en de kaart-tint-botsing vastlegt is langer dan de code die het vervangt. De volgende wijziging tikt de alarmgrens aan — dan is de vraag niet "hoe knip ik bytes" maar of 1120 nog klopt, en niet weer een bump: 1000 → 1050 → 1100 → 1120 is drie bumps in 15 sessies.) (Sessie 218: NEW `terminal-seo.spec.js` met 8 declaraties — de eerste asserties ooit op de content ónder de terminal; vóór deze sessie gaf grep op `terminal-education|edu-command-card|scroll-hint|faq-terminal` in `tests/` **nul** hits). Bundel-limiet 1120 KB, gemeten **1091,02 KB** — marge 28,98 KB (2,6%); de test waarschuwt zelf al op 97,4%. Die grens meet ongeminificeerde broncode sitebreed en is een **driftalarm, geen perf-poort**: Terminal Core (<400 KB minified, de echte poort) wordt met nul bytes geraakt. ⚠️ Sessie 218 voegde ~3,3 KB toe in plaats van te besparen — "content weghalen" is hier géén byte-winst, want de vervanging draagt uitleg, een noscript-vangnet en een trackingmodule.

<!-- VALIDATE-BUNDLE-START Sessie 157 — ground-truth target voor scripts/validate-docs.sh --deep -->
<!-- src=723 styles=434 blog=474 assets=1737 (KB unminified, du -sb / 1024 basis; Sessie 219 ground-truth meting) -->
<!-- VALIDATE-BUNDLE-END -->
...
60. [ ] **`responsive-breakpoints.spec.js:209` overleeft geen parallelle run** (gevonden Sessie 218) — faalt op firefox+webkit zodra drie motoren tegelijk draaien, en is **geen regressie**: serieel gemeten 32/32 groen, 8× per motor per kant tegen `git archive HEAD`, met tijden die op 0,1s gelijk zijn (50,6 vs 50,7s / 35,4 vs 35,5s). Oorzaak is de 10s `toBeVisible`-wachttijd op een JS-geïnjecteerde navbar, die onder CPU-contentie niet gehaald wordt. Bewust niet stilzwijgend verruimd in Sessie 218 (viel buiten die wijziging). Kandidaat-fixes: conditie-wacht op de component-injectie i.p.v. een vaste timeout, of dit blok serialiseren.

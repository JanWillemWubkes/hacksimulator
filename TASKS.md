...
**Status:** M7 Gamification ✅ 100% | M6 Tutorial System 88% | M5.5 Monetization ✅ Live + Brevo deliverability tuned + Gumroad v1.0 + Lead magnet (LIVE on hacksimulator.nl) | Doc-protocol refactor + forcing function (Sessie 140) | Terminal Core ⚠️ ~547 KB minified + ✅ **Lighthouse terminal Mobile 49→59 / Desktop 77→94** ná Pad C1+C2 (Sessie 144, sample-pentest mobile 73→82 / desktop 99→100) + Sessie 147 baseline 74/FCP 1277/LCP 4116 + Sessie 148 main.js dedupe-fix + Sessie 149 #30 Frame D revert (verse mobile baseline 63/4203/816, sync-inline gaf alleen S3 Layout -45 ms hit zonder S6 cascade-weg)
**Sprint:** Sessie 149: Item #30 ⛔ **Frame D revert + spawn #33** — sync-inline navbar+footer compile-time pre-render. Verify-first cyclus volgens plan-file `/home/willem/.claude/plans/heisenberg-hier-cold-start-sessie-wise-book.md` (6 signalen × 3 clusters anti-bias 33,3% symmetrisch, Sessie 147-pattern). **Verse 3-run LH@11 mobile baseline mediaan run-3:** score 63 / LCP 4203 ms / TBT 816 ms / FCP 1916 ms / S3 Top-1 Layout 264,9 ms / S4 LT1 326 ms / Speed Index 4188 ms / CLS 0. Patch (commit `b1c6ded`): terminal.html navbar-placeholder regels 81-96 + footer-placeholder 358-372 vervangen door statische getAppNavbar() + getMarketingFooter() output (+5,6 KB bundle), navbar.js mini-refactor regels 445-489 splits in conditional injection-block + event-binding-switch-altijd voor sync-inline safety. Pre-commit Chromium suite 183 passed, 3 failed + 1 flaky alle 4 onthuld als pre-existing flakes via Sessie 147+148 isolated-rerun tegen productie (cross-browser footer-links, gamification badge tiers, performance VFS NaN, feedback retry). Netlify-deploy <5 sec. **Verse 3-run LH@11 post-mediaan run-3:** score 65 / LCP 4178 (-25, NOISE) / TBT 756 (-60, NOISE) / FCP 1897 (-19, NOISE) / S3 Layout 219,6 (-45, **FRAME A HIT**) / S4 LT1 269 (-57, NOISE) / Speed Index 3981 (-207, niet in plan-thresholds) / CLS ~0. **S6 binair LT1<200ms = false (post 269 ms > 200)** → Frame A criteria gefalsifieerd (S6 vereist true). **Frame B gefalsifieerd:** S3 outside noise-band. **Frame C gefalsifieerd:** geen threshold raak. **Frame D verdict via tie-breaker "Bij twijfel: Frame D"** + cache-coherency-bug-bevinding versterkt revert-keuze (init-components.js importeert navbar.js+footer.js zonder `?v=` cache-bust → returning users 7 dagen lang cached pre-patch navbar.js + nieuwe sync-inline HTML = broken theme-toggle/hamburger/help-dropdown; Sessie 148 #31 pattern op nieuwe locatie ontdekt). **Revert commit `5f0f471`** + deploy 10 sec. **4e mobile-delta-verwachting-falsificatie op rij** (145/146/147/149). Anti-rationalisatie-discipline structureel verankerd. Defense-in-depth 5 plekken: TASKS.md item #30 + current.md Sessie 149 + perf-audit §2d + CLAUDE.md learnings + plan-file outcome-sectie. Spawn #33: structurelere paden (self-host Google Fonts, HTTP/2 server-push deprecation check, CSS critical-path inline, Brotli/compression). Artifacts `/tmp/sessie149-item30/{pre-vector,signals-pre,signals-post,verdict}.json`.
...
32. [ ] **VFS-growth-test NaN-edge-case fix** (Sessie 148 spawn — out-of-scope observatie tijdens #31 spot-check). `tests/e2e/performance.spec.js:496` faalt deterministisch wanneer `avgGrowth === 0` (geen VFS-groei tussen rounds, eigenlijk een GOEDE state) want `stdDev / avgGrowth` = 0/0 = NaN, en `NaN < 0.5` evaluates false → test fails ten onrechte. Comment regel 494 erkende variance-issues maar guarde de avgGrowth=0 edge case niet. **Fix opties:** (a) guard met `if (avgGrowth === 0) { return; }` voor de assertion (geen groei = geen leak = test goed); (b) gebruik `Number.isNaN(ratio) || ratio < 0.5` als assertion. **Scope:** ~5 min + 1 isolated re-run. Geen prioriteit (geen productie-impact, alleen suite-noise).

33. [ ] **Structurelere paden voor LT1-reductie ná #30 Frame D** (Sessie 149 spawn uit #30 closure). Sync-inline cascade-elimination gaf alleen S3 Top-1 Layout -45 ms hit zonder S6 binair cascade-weg (post-LT1 269 ms > 200 ms threshold) → DOM-injection-werk NIET dominant in long-task #1. Bottleneck zit dieper. **Onderzoeks-paden:** (a) **Self-host Google Fonts** — productie laadt 99 KB van fonts.googleapis.com + fonts.gstatic.com via 2 origins met DNS+TLS-handshake elk. Self-hosting vermindert RTT × 2 + render-blocking CSS-import-chain. Caveat: licentie-check + woff2-subsetting voor Inter/JetBrainsMono/SpaceGrotesk. (b) **HTTP/2 server-push deprecation check** — voor de paden waar het nog werkt (browser-support degelijk?). (c) **CSS critical-path inline** — extract top-fold CSS uit main.css+terminal.css naar `<style>` inline, defer rest. Risico: cache-invalidation per HTML-edit ipv per CSS-edit. (d) **Brotli/compression-optimalisatie** — verifieer Netlify levert Brotli-compressed JS/CSS waar mogelijk (default zou aan moeten zijn, maar voorbeeld-check). (e) **Cache-coherency systemic mitigation** — `?v=<sessie>` query-param pattern naar Sessie 148 #31 + Sessie 149 #30 cache-coherency-bevinding uitbreiden tot ALLE module-import URLs in init-components.js + main.js voor toekomstige patches die de import-keten raken (geen blanket-fix nu — alleen toepassen bij volgende relevante patch). **Verify-first cyclus vereist** zoals #29/#30: eigen plan-file + decisional-thresholds-tabel + signaal-clustering vooraf + Frame B/C/D eervolle paden + 4-op-rij-honest-flag pre-emptief. **Geen mobile-delta target.** Alleen mechanisme-bewijs of falsificatie. **Scope:** ~3-4 uur per path. Volgorde TBD obv revisie audit-doc §2d "open vragen". Eerst pad (a) self-host fonts want hoogste-impact-hypothese (99 KB elimination + 2 origins minder + render-blocking-fix).
...
**Laatst bijgewerkt:** 3 jun 2026 (Sessie 149)
**Versie:** 5.3 (Sessie 149 — Item #30 ⛔ **Frame D revert + spawn #33**. Verify-first plan-file `/home/willem/.claude/plans/heisenberg-hier-cold-start-sessie-wise-book.md` met 6 signalen × 3 clusters anti-bias 33,3% symmetrisch (Sessie 147 mirror) + decisional-thresholds-tabel vooraf + Frame B/C/D eervolle paden ingebouwd. **Patch (commit `b1c6ded`):** terminal.html navbar+footer placeholders vervangen door statische `getAppNavbar()` + `getMarketingFooter()` output (+5,6 KB), navbar.js mini-refactor `injectNavbar()` splits in conditional injection + event-binding-altijd voor sync-inline functionaliteit. Lokaal pre-commit alle functioneel-tests groen (themeToggleResponded=true, A11y intact, console 0 errors). Chromium 183 passed; 3 failed + 1 flaky alle pre-existing via isolated-rerun. Netlify <5 sec. **3-run LH@11 mobile mediaan delta:** S1 LCP -25 NOISE / S2 TBT -60 NOISE / S3 Layout -45 FRAME A / S4 LT1 -57 NOISE / S5 FCP -19 NOISE / S6 LT1<200 false → not Frame A. Tie-breaker → Frame D. **Cache-coherency-bug ontdekt parallel:** init-components.js navbar.js + footer.js imports zonder `?v=` → 7-dagen-cache + must-revalidate post-expiration alleen = returning users met cached pre-patch navbar.js + nieuwe sync-inline HTML → no event-binding = broken theme-toggle/hamburger/help-dropdown. Sessie 148 #31 patroon op nieuwe locatie ontdekt. Revert lost dit op. **Revert commit `5f0f471`** + deploy 10 sec. **4e mobile-delta-verwachting-falsificatie op rij** (145/146/147/149). Anti-rationalisatie-discipline structureel volwassen. **Spawn #33:** structurelere paden voor LT1-reductie (self-host Google Fonts hoogste-impact, HTTP/2 push deprecation check, CSS critical-path inline, Brotli verificatie, `?v=<sessie>` cache-coherency systemic mitigation). Geen mobile-delta target. **Defense-in-depth 5 plekken:** TASKS.md item #30 + current.md Sessie 149 + perf-audit §2d + CLAUDE.md learnings + plan-file outcome-sectie. Artifacts `/tmp/sessie149-item30/{pre-vector,signals-pre,signals-post,verdict,extract-signals.py,pw-local-chromium,pw-prod-suspect-rerun,pre-log,post-log}.json+.txt`.)
...
**Bundle (geverifieerd 29 mei 2026, Sessie 144):**
- Site totaal **~1920 KB unminified** | src/ 613 KB + styles/ 262 KB + HTML ~150 KB + blog/ 360 KB + assets/ 685 KB
- **Terminal Core (runtime van terminal.html, gemeten Sessie 141):** **~781 KB unminified** (~547 KB minified geschat) | HTML 19 KB + CSS 160 KB (6 files) + JS module-graph 601 KB (99 files reachable van entry points). ⚠️ ~37% boven 400 KB budget — Pad C1+C2 verlaagt third-party deel, bundle-source ongewijzigd. Volgende sprint: item #26 box-utils.js
- **Lighthouse on-wire ná Pad C1+C2 (Sessie 144, productie):** terminal.html mobile **49→59** / desktop **77→94** (+10/+17), sample-pentest.html mobile **73→82** / desktop **99→100** (+9/+1). Total -251 KB consistent op 4 runs. AdSense 252 KB / 420 ms → **0/0**. terminal.html resterend 3rd-party 101 KB = Google Fonts (0 ms blocking). sample-pentest.html resterend 3rd-party 236 KB = Brevo sibforms (134 KB) + Fonts. Eerlijk-flag: terminal mobile 59 onder verwachte 70-80 door first-party box-utils.js bottleneck. Zie `docs/perf-third-party-audit.md` §7.

...
**Laatst bijgewerkt:** 08 aug 2026 (Sessie 215; hero-terminal uitlijning + focusrand + uitnodiging)
...
**Current Status:** ✅ LIVE — Playwright E2E: **34 spec files / 273 `test()`-declaraties** (Chromium, Firefox, WebKit). Let op: het aantal *gedraaide* tests ligt hoger dan het aantal declaraties sinds Sessie 212 — `lead-magnet.spec.js`, `navbar-collapse.spec.js` (Sessie 213) en sinds Sessie 215 drie blokken in `hero-demo.spec.js` genereren hun scenario's in een `for…of`, dus 1 declaratie = 2-3 tests. Sessie 215-verificatie: **105 passed / 0 failed** over `hero-demo` + `homepage-conversion`, drie engines, geen flakes. | Ko-fi + Brevo (deliverability getuned) + Gumroad v1.0 + **2 lead magnets met elk een eigen Brevo-formulier + automation (Sessie 212)** | **advertenties volledig verwijderd (Sessie 208)**
...
- **E2E-bundle-test-limiet (performance.spec.js): 1050 → 1100 KB (Sessie 214).** Eerder 1000 → 1050 (Sessie 204, jul 2026) na organische groei door funnel/analytics-werk Sessie 190-203. Aanleiding nu: de interactieve hero-terminal (`src/ui/hero-repl.js` + hero-CSS + markup) kostte 24 KB tegen 4,78 KB resterende ruimte; gemeten ná de wijziging **1069,20 KB**. De test meet unminified source (src/ JS + styles/ CSS + src/ui CSS + index.html) en is dus een **driftalarm**, geen perf-poort: de echte poort blijft Terminal Core <400 KB minified, en die wordt met nul bytes geraakt — `hero-repl.js` zit alleen in de module-graph van `index.html`, niet in die van `terminal.html` (zie regel hieronder + #24). Marge bewust ~31 KB (2,9%): een limiet die vlak boven de huidige stand ligt vuurt op élke wijziging en wordt dan weggeklikt in plaats van onderzocht. **Stand Sessie 215: 1078,05 KB — marge 20,95 KB (2,0%).** De +8,8 KB is grotendeels commentaar (landing.css +6,3 KB); de prosa is al uitgedund met behoud van elke meting en valkuil. De volgende wijziging tikt de alarmgrens aan: dan is de vraag niet "hoe knip ik bytes" maar of 1100 nog klopt.
...
- **Playwright:** 34 spec files, 273 `test()`-declaraties per browser-project (Sessie 215 added +9 in `hero-demo.spec.js`: uitlijningsinvariant (midden-verschil ≤2px + `margin-top: 0px`), mobiele kolomvolgorde, focustoestand geparametriseerd over dark/light — dát is de assertie die de bronvolgorde-val vangt —, hint binnen zijn kolom @769/@1280, chipbedekking-baseline @375×812/@390×844, hint zichtbaar-dan-weg zonder sprong, cursornabijheid, promptregel-tikdoel, veldbreedte na overname. Alle negen met de mutant rood bewezen. Sessie 213 added +14 in NEW `gidsen-layout.spec.js` (8: CTA-uitlijn-invariant, weeskaart-check desktop+tablet, mobiel 1-koloms, sample-chips + tap target, chip-kleuren light/dark, en 3 regressiewachten op gedeelde CSS) en NEW `navbar-collapse.spec.js` (6: overflow per marketingpagina, omslagpunt 1279/1280, overlay-gedrag, Escape/toggle-sluiting, CTA-onderscheid, terminal-navbar ongemoeid). Beide suites eerst rood tegen productie gedraaid. Sessie 209 added +6 in NEW `w2-verification.spec.js`: skip-certificaat DEELNAME vs VOLTOOIING (×2), helpsysteem-escalatie (×2), localStorage-backward-compat (×2). Sessie 205 added +1 "Live resize reflow"-test: render @1240 → live versmallen naar 900/700/640 → 0 wraps + scroll blijft gepind; groen op alle 3 engines. Sessie 204 added +4 tussenbreedte-tests 800/900/1024/1100 in responsive-ascii-boxes.spec.js + echte wrap-detector; Sessie 197 added +2 in NEW `persistence-flush.spec.js`: flush-on-`visibilitychange(hidden)` bewaart challenge-voltooiing + VFS-mutatie binnen het 500ms-debounce-venster. Chromium-baseline Sessie 197 tegen lokale werkkopie: 184 werkkopie-specs groen vóór+na de fixes; 49 "failures" = 10 prod-hardcoded specs (egress `000`) + 5 tutorial-gestures (iPhone-13-device-emulatie-harnasartefact) — géén werkkopie-regressie. Sessie 196 added +2 in gamification.spec; Sessie 195 added +4)

<!-- VALIDATE-BUNDLE-START Sessie 157 — ground-truth target voor scripts/validate-docs.sh --deep -->
<!-- src=716 styles=425 blog=474 assets=1737 (KB unminified, du -sb / 1024 basis; Sessie 215 ground-truth meting) -->
<!-- VALIDATE-BUNDLE-END -->
...
22. [ ] Postmaster re-check trigger: eerste >100-recipient campaign-send OF kalender-datum 2 wk later (vanaf 18 mei 2026 → ~1 juni 2026) — **⏰ trigger verstreken (gecheckt 31 jul 2026, Sessie 204): actie bij Heisenberg** (postmaster.google.com/managedomains)
...
56. [ ] **`.mobile-cta-bar`: verschijnen koppelen aan "geen andere CTA in beeld"** — gemeten in Sessie 215 over zes telefoonmaten (consent gezet, dus dit meet de balk en niet de cookiebanner): bij scrollpositie 0 staat de hero-CTA op **élke** maat al in beeld, dus de vaste balk voegt daar niets toe — en dekt wél af. Op **360×800 en 390×844** liggen `whoami`, `pwd` en `help` eronder; op 390×844 staan bovendien twee identieke groene "Start de simulator"-knoppen tegelijk op één scherm. Pre-existing sinds Sessie 214 (chips); oud en nieuw byte-identiek gemeten via `git archive HEAD` op een tweede poort. `landing.css:2537` stelt de bedoeling al ("De balk mag niets afdekken") maar lost hem op met `body { padding-bottom: 76px }`, wat alleen garandeert dat je alles kúnt wegscrollen. **Waarom niet in Sessie 215 gedaan:** elke echte fix raakt de conversiegarantie, niet de chips. Een IntersectionObserver maakt die eigenschap tijdsafhankelijk en breekt `homepage-conversion.spec.js:153`, dat synchroon in één `page.evaluate` scrollt waar observer-callbacks nooit vuren — die guard moet dan bewust async herschreven worden, en minstens zo streng blijven. Het CSS-alternatief (`position: sticky; bottom: 0` in de flow ná de hero) heeft geen JS en geen timing, maar kost 65px echte flow-ruimte plus een monetisatie-element dat in de DOM verhuist. **Klaar als:** `BASELINE_BEDEKT` in `tests/e2e/hero-demo.spec.js` voor beide maten `[]` wordt (die test is precies daarvoor geschreven), de CTA-garantie aantoonbaar even streng blijft, en er nooit twee identieke CTA's tegelijk in beeld staan.
...
- [ ] Postmaster Tools re-check — trigger: eerste >100-recipient campaign-send OF ~1 juni 2026 (Sessie 136 vastgesteld) — **⏰ trigger verstreken, actie bij Heisenberg (Sessie 204)**

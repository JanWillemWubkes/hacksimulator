...
**Laatst bijgewerkt:** 20 aug 2026 (Sessie 229; JetBrains Mono ligeerde `>=` naar ≥ en `$pdo->prepare` naar `$pdo→prepare` — sitebreed uitgezet, nieuwe pixel-guard)
...
- **Playwright:** **45 spec files, 316 `test()`-declaraties** per browser-project — **Sessie 229: +1 spec / +2 declaraties** (`font-ligatures.spec.js`). Gemeten met `grep -rE "^\s*test\("`, niet uitgerekend, en opnieuw is de declaratie-telling misleidend: die 2 declaraties genereren **31 tests** (één `test()` in een `for…of` over de 30 pagina's uit `helpers/paginas.js`, plus de pixelvergelijking). Sessie 229-verificatie: volle suite over **drie motoren** **1520 passed / 0 failed / 25 skipped** (1,2 uur). ⚠️ De eerste poging gaf exit 0 mét `55 did not run` — een zelfgekozen `--global-timeout` van 25 min kapte de run af, en 451-passed-in-25,0-min las als groen. Een afgekapte run is geen groene run; de limiet hoort op een **meting** te staan (chromium alleen heeft ~28 min nodig), niet op een gevoel. Zie ook #73.

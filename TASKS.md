...
**Laatst bijgewerkt:** 5 sep 2026 (Sessie 232; de bloat zat niet in de code maar in twee tests die alleen een modal wegklikten en een gitignored map van 55 MB)
...
**Versie:** 6.02 (Sessie 229 — **Het font schreef iets anders dan de DOM.** JetBrains Mono ligeert via `calt` (367 lookups, standaard aan) en er stond nergens een `font-variant-ligatures`-regel: `>=` rendeerde als ≥, `_|_` als ⊥, en `$pdo->prepare` als `$pdo→prepare` — dat laatste in het voorbeeld onder "Veilige code:", dus wie overtypte kreeg een parse-error. De bron klopte al; `textContent` gaf `>=`. **Ligaturen behouden het monospace-grid exact (delta 0,00px), dus breedte detecteert dit niet — alleen pixels.** Populatie omgedraaid (`*, *::before, *::after`) i.p.v. een lijst van 48 mono-declaraties; prose-kosten gemeten op 0,17px (Space Grotesk) en 0,00px (Inter). NEW `font-ligatures.spec.js` (2 asserties, 3 mutanten met 3 faalsignaturen) + `helpers/paginas.js`. De zelfbewakende tak ving direct een eigen meetfout: `<pre>` krijgt een UA-`font-family` die overerving verslaat. 45 specs / 316 decl; 1520 passed over drie motoren. Bundel 1104,62/1120 KB. ⚠️ Een eerste volle run gaf exit 0 mét `55 did not run` — een global-timeout op gevoel i.p.v. op een meting.)

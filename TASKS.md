...
67. [ ] **`assets/legal/terms.html` heeft 22px horizontale overflow @375px + de drie legal-pagina's hebben géén E2E-dekking (Sessie 223)** — **A/B tegen HEAD uitgevoerd** (`git archive` + twee poorten): 22px aan beide kanten, dus **pre-existing**, niet veroorzaakt door Sessie 223. Gemeten: `scrollWidth` 397 tegen `clientWidth` 375, terwijl **geen enkel element** met zijn border-box buiten beeld valt — vermoedelijk een pseudo-element of scroll-regio, oorzaak niet vastgesteld. De echte bevinding is het **dekkingsgat**: `privacy.html`, `terms.html` en `cookies.html` komen in géén enkele overflow-assertie voor (de bestaande specs dekken index, gidsen, over-ons en één blogpost), en dát is waarom dit ongezien bleef. Fix de dekking vóór de 22px — een spec meldt terug, een notitie niet.

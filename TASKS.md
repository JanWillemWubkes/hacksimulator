...
**Laatst bijgewerkt:** 15 sep 2026 (Sessie 234; de mailbox kostte ~EUR 96/jaar, en de gratis standaardroute sterft in januari 2027)
**Sprint:** Sessie 234: **De mailbox kostte ~EUR 96/jaar en de provider had geen kleiner pakket** (`82a9a08`, `c058577`, gepusht). Opdracht: kan `contact@hacksimulator.nl` gratis elders? TransIP-support bevestigde dat er niets tussen een vol pakket en pure forwarding bestaat. **De gratis standaardroute bleek stervend:** forwarden naar Gmail en van daaruit antwoorden houdt op in **januari 2027** — Google schrapt "Send mail as" voor niet-Google-adressen en beperkt nieuwe configuraties nu al. Voor een domein dat security-disclosures en AVG-verzoeken ontvangt is antwoorden vanaf een prive-Gmail geen optie, dus forwarding viel af. **Zoho Mail Free op het EU-datacenter (Nederland)** geworden: `contact@` en `dmarc@` als aliassen op een superuser (30 aliassen toegestaan op het gratis plan, tellen niet mee voor de 5 gebruikers), 2048-bit DKIM, MX naar `mx.zoho.eu` 10/20/50. **De regio hangt af van de ingang, niet van je IP:** dezelfde knop op dezelfde pagina wijst naar `workplace.zoho.com` als je via `zoho.com/nl` binnenkomt en naar `workplace.zoho.eu` via `zoho.eu` — twee keer uitgelezen in de browser. De `?sredirect=true` in de URL is het enige zichtbare bewijs, en de keuze ligt onherroepelijk vast bij het aanmaken. **Brevo slaagt op DKIM, niet op SPF:** de Return-Path is Brevo's eigen bounce-domein, dus `include:spf.brevo.com` wordt voor die mail niet eens geraadpleegd — mijn eerdere waarschuwing dat de nieuwsbrief zou breken zonder die include was **onjuist**; de twee `brevo*._domainkey`-CNAMEs doen het werk. **Vijf TransIP-records opgeruimd tegen een enkel Zoho-record**, en dat is geen tekort: `autoconfig`/`autodiscover` hebben zonder IMAP geen functie (en Zoho doet autodiscovery met SRV/XML, niet met CNAME), en drie provider-beheerde DKIM-selectors voor sleutelrotatie werden een zelfbeheerde TXT. **`check-mail.sh` bewaakt beide kanten** — wat moest veranderen (MX, SPF, DKIM, DMARC) en wat intact moest blijven (Brevo-DKIM, A-record, Google-verificatie); de DKIM-tak decodeert de sleutel met OpenSSL in plaats van te kijken of er iets staat, want een afgekapt 2048-bit record geeft `dkim=fail` en is schadelijker dan geen DKIM. De eerste versie had een guard die **niet kon falen**: een negatieve lookahead in `grep -E` viel terug op een lege string en gaf groen op een MX die nog naar TransIP wees. **privacy.html:** Zoho in de verwerkerstabel tussen Netlify Forms en Brevo, plus paragraaf 11 (EU-datacenter, geen doorgifte buiten de EER) — en onderweg bleek de footer `v1.0 / 16 okt 2025` te claimen terwijl de kop `v1.1 / 24 aug 2026` zei: elf maanden drift in een juridisch document.
**Sprint:** Sessie 233: **De launch-visuals waren 38 dagen oud, en de verse capture legde een renderbug bloot** (`417fa53`, `e50630e`, `86d958d`, gepusht). Opdracht: wat is de volgende stap? Mijn eerste antwoord (de vier Firefox-flakies) was **fout gerangschikt** en Heisenberg wees me erop: de site is 38 dagen live, drie aankondigingskanalen zijn nooit afgevuurd en de D+14-meting is nooit gedaan, terwijl sessies 221-232 allemaal technisch waren. Ik optimaliseerde voor wat ik vanuit de repo kan méten, niet voor waarde. **De launch-kit fire-ready gemaakt** — en de her-capture van de visuals werd de sessie. **De oude take bleek van vóór 6 juli**, met twee onafhankelijke tells: hij draagt nog de `[?] TIP:`-marker die `43eeb58` op 6 juli hernoemde (82 hits, 27 bestanden) én de GIF is 640 hoog terwijl het script sinds 14 juli 720 zegt. De her-capture van Sessie 173 is dus nooit in de veilige map beland, en TASKS.md tekende *"cyaan `[TIP]` geverifieerd"* af op een beeld dat het tegendeel toont. **De "vreemde cirkels" die Heisenberg meldde waren GIF-dithering, geen site-bug:** `.grid-background` is een echte vignette, verliesvrij vloeiend (74 tinten, ramp 4→17), maar in de GIF nog 2 kleuren met 60 harde overgangen — het 256-palet gaat op aan de groene tekst, en de dither-dichtheid springt met de straal (100 → 91 → **40** → 18%). Nu platgeslagen in alleen de GIF-capture: **1 kleur, 0 overgangen**. **Bijvangst die de sessie kaapte (#77):** de terminal rendeerde `[→]` in Chromium als `→]`. Zelfde beeld, alle drie op x=60 — `[✓]` inkt 185, `[TIP]` 255, `[→]` **9**. Chromium-only; overleefde scroll, repaint én relayout. **Oorzaakketen:** de font-subset miste U+2192/U+2190/U+2713, die vielen terug op een systeemfont met een andere baseline, `renderer.js` compenseerde met een opgetilde span, en die liet `[` achter als tekst-run van één teken vóór een elementgrens. **Gerepareerd in het brondocument:** subset opnieuw gebouwd uit upstream JetBrains Mono (SIL OFL) — 232 codepoints, niets kwijt, **0 advance-width-verschillen** over de 229 gedeelde, bestand 31.432 → **30.272 bytes**. Daarna kon `.marker-arrow` volledig weg. Pijl t.o.v. haak nu chromium +0,5 / firefox 0,0 / webkit +0,5 px. NEW `marker-brackets.spec.js`, 3 mutanten op 3 asserties. Regressie **251 passed / 0 failed** over drie motoren. Volledig: `docs/sessions/current.md`.
**Sprint:** Sessie 232: **De bloat zat niet in de code — twee "debugtests" klikten alleen een modal weg en asserteerden verder niets** (`32cafeb`, `a8f4ceb`, `7fa390d`, gepusht). Opdracht: analyseer het project op bloat. **De code bleek schoon en dat is de hoofdbevinding:** 0 verweesde modules van 118 in `src/`, 0 van 11 stylesheets, 0 van 36 bestanden in `assets/`, 0 md5-duplicaten over 387 getrackte bestanden. Wat overbleef: **153M → 53M op schijf**. `.playwright-mcp/` stond op **55 MB in 1111 screenshots** over vijf maanden — meer dan alle broncode, docs en assets samen — en `git gc --prune=now` bracht 3388 losse objecten (44,31 MiB) naar één pack: **.git 69M → 26M**. **Twee tests konden niet falen op hun eigen onderwerp:** `debug-console.spec.js` (2 expects, 15 console.logs) en `debug-storage.spec.js` (4/15) — álle zes expects klikten enkel het legal-modal weg als setup; nul asserties over console-errors of localStorage, terwijl de echte dekking assertief in `persistence-flush.spec.js` en `vfs-versioning.spec.js` staat. `modal-colors-simple.spec.js` (2 expects) was een strikte subset van `modal-headers.spec.js` (7). **Grootste getrackte bestand was een dood meetartefact:** `lighthouse-m9-baseline.json`, 637 KB, groter dan TASKS.md, en de enige verwijzing ernaar was een uitsluitingsregel in `.gitleaks.toml`. Van de vier gitleaks-exclusies bleek na meting maar één exclusief van dat bestand afhankelijk — `ca-pub-6345664385525701` staat óók in `archive-s175-s179.md`, dus beide schrappen had CI rood gemaakt. NEW **Check 20** in `validate-docs.sh`: `20a` warnt vanaf 10 MB en faalt vanaf 50 MB (twee drempels, want een volle map is rommel en geen defect — alleen falen leert je `--no-verify`), `20b` faalt zodra er iets getrackt onder die paden staat, want dán gooit de nieuwe opruimstap in `/summary` Step 7 werk weg i.p.v. rommel. Beide dragen een ijkmeting omdat de mappen er meestal niet zijn en "0 MB" anders niet te onderscheiden is van een stukgelopen meting. **5 mutanten, 5 verschillende asserties**, en de warn-tak geverifieerd op exit 0. ⚠️ **Eigen meetfout, en niet de eerste van deze soort:** mijn eerste volle suite rapporteerde exit 0 met **862 did not run** — dat was `tail`'s exit-code, niet die van Playwright, en mijn eigen `--global-timeout` van 30 min kapte de run af. Sessie 229 documenteerde exact dit patroon al (`55 did not run`); een sessielog laadt niet mee, dus de regel staat nu in `.claude/rules/meten-en-guards.md`. Correcte eindmeting: **1520 passed / 0 failed / 0 did not run** (1.3h), 3 flaky op firefox.
**Sprint:** Sessie 231: **`publish = "."` zette de bron van vier betaalde gidsen op de CDN** (`4f596ee`, `b7a73c9`, `99f0b62`, `6a18b93`, `3c4e897`, `d2e4185`, `8d7397f`, `ecaf0b2`, gepusht). Achteraf gereconstrueerd uit de commits; deze sessie kreeg destijds geen `/summary`, terwijl 8 codebestanden zichzelf al "Sessie 231" noemen. **Er is geen build-stap, dus de publish-root is de repo-root:** alles wat git trackt werd geserveerd. Gemeten op productie, allemaal **HTTP 200**: `pentest-playbook.typ` (24.584 B), `leerplan.typ` (36.373 B), `lab-opzetten.typ` (31.067 B), `juridische-gids.typ` (17.830 B) — de volledige inhoud van de vier Gumroad-gidsen, gratis naast de betaalde PDF. Daarnaast stond `archive-s121-s164.md` live (388 KB) met drie privé-mailadressen. `robots.txt` had `Disallow: /docs/`, maar dat is indexeringsadvies, geen toegangscontrole. **Guards erbij met de populatie omgedraaid:** Check 19a eist niet "staan de paden die ik ken in een blokkeerlijst" maar "élke top-level entry die git trackt wordt geserveerd, dus verantwoord je" — een lijst-guard bewaakt zijn lijst, en precies daardoor kon `docs/products/` meeliften. 19b vangt privé-mailadressen als klasse. **Check 19 betrapte in zijn eerste CI-run de commit die hem introduceerde:** `package-lock.json` uit `.gitignore` halen maakte er een getrackte top-level entry van. Lokaal groen om twee redenen die beide zijn gerepareerd: validate-docs draaide vóór `git add` (Check 19 leest `git ls-files`), en de pre-commit-hook draaide helemaal niet omdat zijn `files:`-patroon `netlify.toml` noch `.gitignore` dekte. **450 KB derde-partij-JS dat niets meer deed:** Brevo's `main.js` stond op vier pagina's en laadde vóór elke toestemmingsvraag, terwijl `brevo-submit.js` het submit-event in de capture-fase onderschept met `stopImmediatePropagation()` — Brevo's eigen handler kwam er niet meer aan te pas. Ablatie gemeten i.p.v. beredeneerd: `main.js` render byte-identiek zonder (zelfde MD5) → weg; `sib-styles.css` (57,6 KB) verschuift de kaart 74px → blijft, en staat nu expliciet in het privacybeleid. **`try/catch` dekt kapotte JSON, niet geldige JSON van de verkeerde vorm:** `"hoi"`, `[]` en `null` parsen prima en komen dus nooit in de catch — drie localStorage-lezers indexeerden ze daarna als object. De fallback bestond al in alle drie de gevallen, hij werd alleen niet bereikt. Verder: de rotatieformule uit `/summary` verwijderd ten gunste van een verwijzing naar de eigenaar (`docs/sessions/README.md`), want de kopie was twee keer gecorrigeerd en verhuisde beide keren mee; de AI-tooltip uit 15 blogposts (gematcht op de exacte 107-byte string, niet op `title=`, want dezelfde pagina's dragen 149 `abbr`-jargontooltips die een brede strip zou hebben meegenomen); en een CTA op `over-ons.html` die "Direct aan de slag" beloofde en één regel later "Nieuw met hacken? Lees eerst ..." zei.
**Sprint:** Sessie 230: **Het nieuwsbriefblok viel buiten de filterpopulatie — en rekte via één grid-track alle 15 kaarten op** (`e55f21e`, gepusht, live geverifieerd). Aanleiding was een melding dat `/blog/#gevorderden` bovenaan het inschrijfformulier toont en geen enkel artikel. **Gemeten op de live site bleek dat geen randgeval maar 4 van de 6 categorieën:** het `:target`-filter verbergt uitsluitend `.blog-post-card`, en het nieuwsbriefblok staat als 4e kind ín `.blog-posts-grid` zónder `data-category`. De drie kaarten ervóór zijn `beginners`, `tools`, `tools`, dus élke categorie waarvan de eerste match ná het blok valt (`concepten` 3, `carriere` 1, `bronnen` 1, `gevorderden` 1) kreeg het formulier bovenaan; `beginners` en `tools` waren toevallig goed. **Fix:** groep 4 in het `:target`-blok, `order: 1` zodra er gefilterd wordt — klasse-gebaseerd op `.category-target`, dus anders dan groep 1 t/m 3 groeit hij vanzelf mee met een nieuwe categorie. Ongefilterd blijft het blok op index 3 staan: naar de gridbodem is **5458px dieper** op een pagina van 7361px. **Twee stille defecten in hetzelfde blok kwamen tijdens het meten boven.** (a) `width: 280px` op de Brevo-input versloeg de mobiele `width: 100%` op specificiteit ((0,3,1) vs (0,2,1)) en gaf het blok een **min-content van 400px** — het enige grid-item boven 360px. Eén impliciete `auto`-track betekent dat het breedste item de rest sleept, dus **alle 15 kaarten renderden 400px in een container van 336px**, en `main.blog-container{overflow-x:hidden}` knipte die 64px onzichtbaar weg (kaartrand 412 → 348 = containerrand; min-content 400 → 232). (b) `blog-filter.js` las élke hash als categorie, dus `/blog/#main-content` — het doel van de skip-link — meldde "0 van 15 artikelen" in een `role="status"`-regio terwijl CSS alle 15 toonde. NEW 3 guards in `blog-navigation.spec.js`, elk met zelfbewakende tak; drie mutanten vuren op drie verschillende asserties. Zie #76.
**Sprint:** Sessie 229: **De terminal toonde iets anders dan er in de DOM stond — het font ligeerde `>=` naar ≥ en `$pdo->prepare` naar `$pdo→prepare`** (`aab57a2`, `3d3a228`, gepusht). Aanleiding was een vraag over "vreemde tekens" in de sqlmap-output. **Diagnose:** de bron was correct — `textContent` gaf keurig `>=` terug — maar JetBrains Mono ligeert via de OpenType-feature `calt` (**367 lookups**, gemeten met fontTools op `jetbrainsmono-latin.woff2`), en `calt` staat in browsers standaard aan. Nergens in `styles/` stond een `font-variant-ligatures`-regel (0 treffers). Gerenderd werd: `>=` → ≥, `_|_` → ⊥, `-|` → ⊣, `->` → →. **De zwaarste vindplaats was niet de banner maar `man sqlmap`:** onder het kopje "Veilige code:" rendeerde `$pdo->prepare(` als `$pdo→prepare(`, direct onder het onveilige voorbeeld dat met `[X]` is gemarkeerd — wie overtypte wat hij zag kreeg een PHP-parse-error, bij precies het voorbeeld dat als het juiste alternatief wordt aangewezen. Dat maakte dit een leerbug, geen cosmetiek. **Fix:** populatie omgedraaid (`*, *::before, *::after`) i.p.v. een lijst mono-selectors — `--font-terminal` staat in **48 declaraties over 7 stylesheets**, en zo'n lijst bewaakt zichzelf, niet de klasse; drift valt nu in de goedkope richting. Kosten in prose gemeten op 40px, normal vs none: Space Grotesk max **0,17px**, Inter **0,00px** — sub-pixel, dus géén uitzonderingslijst. **Breedte kan dit niet detecteren:** JetBrains Mono-ligaturen behouden het monospace-grid exact (gemeten delta **0,00px** tussen `>=` en ≥), dus de guard moet pixels vergelijken. NEW `tests/e2e/font-ligatures.spec.js` met twee asserties (sitebrede dekkingssweep + pixelvergelijking met zelfbewakende tak) en `tests/e2e/helpers/paginas.js` (paginalijst uit `text-contrast.spec.js` gehaald). Bijvangst: de sqlmap-banner bleek het enige output-blok van de 41 commands zonder `← NL-context` of `[TIP]` — één regel toegevoegd in beide takken. Zie #75.
...
- **E2E-bundle-test-limiet (performance.spec.js): 1120 KB** — gemeten stand **1105,32 KB** (Sessie 233, marge **14,68 KB = 1,3%**; +480 bytes, uitsluitend commentaar bij de font-fix — het woff2 zelf telt niet mee in `RUNTIME_SOURCE` maar werd 1.160 bytes kleiner). **Sessie 230-delta: 0,00 KB**, en dat is geen toeval maar de Sessie 227-uitsluiting die werkt: `styles/blog.css` (+2,53 KB commentaar) en `src/ui/blog-filter.js` (+0,75 KB) vallen in de **Blog-pijler (budgetloos, alleen ter informatie)** — die stond na deze sessie op 55,56 KB. ⚠️ Dat was vooraf een aanname en is achteraf gemeten door `performance.spec.js` te draaien vóór het advies: met een marge van 15,38 KB is "blog telt toch niet mee" precies het soort bewering dat je niet moet geloven zonder cijfer. De formule zelf is in Sessie 227 gerepareerd (#70: blog-assets uitgesloten, dode `src/ui/**/*.css`-term weg, `terminal.html` erbij) en is sindsdien ongewijzigd. De delta van deze sessie is **+1,00 KB** t.o.v. de 1103,62 KB waarmee Sessie 229 begon: +0,77 KB voor de ligatuurregel mét rationale in `main.css`, +0,23 KB voor de twee `[TIP]`-regels in `sqlmap.js`. ⚠️ Het CSS-commentaar stond binnen deze sessie eerst op +1,69 KB en is gehalveerd door de volledige meting (mutanten, prose-kosten, de reden dat breedte niet detecteert) naar `tests/e2e/font-ligatures.spec.js` te verplaatsen — dat bestand telt niet mee — en in de CSS alleen het gemeten cijfer plus een verwijzing te laten staan. Dat is dezelfde remedie als in Sessie 228 en ze werkt nog steeds: het verhaal hoort in `tests/`, `docs/` en dit bestand, de CSS houdt het cijfer. Zie #74 voor de minify-trigger; die staat nog niet aan (marge > 5 KB, en de groei is CSS-commentaar, geen JS).
...
75. [x] **Het font schreef iets anders dan de DOM: `calt`-ligaturen stonden sitebreed aan ✅ Sessie 229** — commits `aab57a2`, `3d3a228`. Aanleiding: een vraag over "vreemde tekens" in de sqlmap-output. **De bron was correct** — `textContent` gaf op de live site keurig `>=` terug — maar JetBrains Mono ligeert via de OpenType-feature `calt` (**367 lookups**, gemeten met fontTools op `jetbrainsmono-latin.woff2`), die standaard aan staat, en er stond nergens in `styles/` een `font-variant-ligatures`-regel (0 treffers). Gerenderd: `>=` → ≥, `_|_` → ⊥, `-|` → ⊣, `<=` → ≤, `!=` → ≠, `=>` → ⇒, `->` → →. **De zwaarste vindplaats was niet de banner maar `man sqlmap`:** onder het kopje "Veilige code:" rendeerde `$pdo->prepare(` als `$pdo→prepare(` en `$stmt->execute(` als `$stmt→execute(` — direct onder het onveilige voorbeeld dat met `[X]` is gemarkeerd. Wie overtypte wat hij zag kreeg een PHP-parse-error, bij precies het voorbeeld dat als het juiste alternatief wordt aangewezen. Leerbug, geen cosmetiek. **Waarom dit drie sessies onopgemerkt bleef:** de schade zat óók in de sqlmap-banner, maar een corrupte glyph in ASCII-art ziet er niet corrupter uit dan een correcte — pas waar de tekens betekenis dragen was het ondubbelzinnig. **Fix:** populatie omgedraaid (`*, *::before, *::after { font-variant-ligatures: none }`) i.p.v. een lijst mono-selectors: `--font-terminal` staat in **48 declaraties over 7 stylesheets**, en zo'n lijst bewaakt zichzelf, niet de klasse; nu valt drift in de goedkope richting (een prose-element mist een fi-ligatuur) i.p.v. de dure (een nieuwe mono-context toont weer `$pdo→prepare`). **Kosten van die keuze gemeten** i.p.v. beredeneerd — Space Grotesk draagt `liga` (22 lookups), Inter `calt` (43), dus een blanket-disable is niet gratis: breedtedelta op **40px** tekst is Space Grotesk max **0,17px**, Inter **0,00px**. Sub-pixel, dus géén uitzonderingslijst. **De meetval die de guard-opzet bepaalt:** JetBrains Mono-ligaturen behouden het monospace-grid **exact** — gemeten delta tussen `>=` en ≥ is **0,00px** — dus een guard op breedte óf op `textContent` is groen bij een kapotte render. Alleen pixels bewijzen hier iets. **NEW `tests/e2e/font-ligatures.spec.js`**, twee asserties die anders falen: (A) sitebrede dekkingssweep, populatie = álle elementen, wat mono is moet `none` hebben, faalt óók op een lege populatie; (B) pixelvergelijking van een probe in de echte terminal, met een **zelfbewakende tak** die eist dat `normal` en `none` verschíllen. **Drie mutanten, drie faalsignaturen:** M1 (regel weg) 30 failed / 1 passed — A op 29 pagina's + B tak 2; M2 (regel gescopet naar `#terminal-output`) 29 failed / 2 passed — A alleen, B groen; M3 (`woff2` hernoemd) 1 failed / 30 passed — B tak 1 alleen, A groen. **Twee meetfouten die de guard over zichzélf aan het licht bracht:** (a) de probe was een `<pre>`, en de UA-stylesheet zet daarop een eigen `font-family: monospace` die overerving verslaat — de probe mat de generieke browser-monospace, die niet ligeert, dus tak 2 zou groen zijn geweest zonder iets te bewijzen; (b) de legal-modal krijgt `active` pas rond **800ms**, dus een `isVisible()` op t=0 slaat hem over en dan appendt de terminal **nul** regels (6 s later nog leeg). Beide opgelost, en (a) is nu ook een eigen assertie. **Cross-browser geverifieerd:** 62/62 op Firefox + WebKit — `font-variant-ligatures: none` werkt daar unprefixed, en dat tak 1 óók op WebKit vuurt bewijst dat daar écht ligaturen aan stonden. `?v=228`→`?v=229` op **79 verwijzingen over 30 HTML-bestanden** (79 in, 79 uit, niets anders geraakt). **Bijvangst:** `PAGINAS` uit `text-contrast.spec.js` naar NEW `tests/e2e/helpers/paginas.js` — twee sweeps over dezelfde site horen niet elk hun eigen lijst te dragen. **Tweede bijvangst (`3d3a228`):** de sqlmap-banner bleek het enige output-blok van de 41 commands **zonder** `← NL-context` of `[TIP]`, terwijl `command-output.md` dat voorschrijft — sqlmap is ook de enige command met ASCII-art (8 bannerregels), dus er was geen precedent. Eén `[TIP]`-regel toegevoegd in **beide** takken (kwetsbaar én niet-kwetsbaar). Eerste formulering brak de zin middenin; op 375px viel die breuk samen met de soft-wrap en las het als twee losse blokken. Nu is elke bronregel een hele zin, dus de wrap kan geen fragment losknippen — gemeten: desktop 1+1 rijen, 375px 3+2 rijen, geen horizontale overflow, beide regels `rgb(121,192,255)`.
76. [x] **Het nieuwsbriefblok viel buiten de filterpopulatie — en rekte via één grid-track alle 15 kaarten op ✅ Sessie 230** — commit `e55f21e`, gepusht en live geverifieerd (33/33 tegen productie). Aanleiding: een melding dat `/blog/#gevorderden` bovenaan het inschrijfformulier toont en geen enkel artikel. **Gemeten op de live site was dat geen randgeval maar 4 van de 6 categorieën.** Het `:target`-filter verbergt uitsluitend `.blog-post-card`; het nieuwsbriefblok staat als 4e kind ín `.blog-posts-grid` en draagt géén `data-category`, dus het bleef in élke stand staan waar het stond. De drie kaarten ervóór zijn `beginners`, `tools`, `tools` — dus élke categorie waarvan de eerste match ná het blok valt kreeg het formulier bovenaan: `concepten` (3), `carriere` (1), `bronnen` (1), `gevorderden` (1); `beginners` en `tools` waren toevallig goed. **Fix:** groep 4 in het `:target`-blok, `order: 1` zodra er gefilterd wordt. **Klasse-gebaseerd** op `.category-target` i.p.v. zes id-selectors erbij: groep 1 t/m 3 móéten enumereren (er is geen selector die `[data-category]` aan een target-id koppelt), deze niet — hij groeit vanzelf mee met een nieuwe categorie, en de ⚠️-comment erboven zegt dat nu expliciet. Ongefilterd blijft het blok op index 3: naar de gridbodem is **5458px dieper** op een pagina van 7361px, dus "gewoon onderaan zetten" gooit de inschrijving praktisch weg. **Prijs, bewust betaald en als commentaar vastgelegd:** in gefilterde standen wijkt de DOM-volgorde af van de visuele. De node in JS verplaatsen zou dat óók repareren, maar breekt de belofte in `blog-filter.js` dat het filter zonder JS volledig werkt — en beide volgordes zijn betekenisbehoudend (twee losse blokken, geen omgedraaide leesvolgorde bínnen één component). **Twee stille defecten in hetzelfde blok kwamen tijdens het meten boven.** (a) **Eén grid-item rekte alle 15 kaarten op:** `width: 280px` op de Brevo-input (twee keer gedeclareerd) versloeg de mobiele `width: 100%` op specificiteit — (0,3,1) tegen (0,2,1), en een media query voegt er geen toe. Dat gaf het blok een **min-content van 400px**, het enige grid-item boven 360px; `.blog-posts-grid` heeft één impliciete `auto`-track, dus het breedste item sleept de rest mee en **alle 15 kaarten renderden 400px in een container van 336px**. `main.blog-container{overflow-x:hidden}` knipte die 64px onzichtbaar weg — daarom heeft niemand het ooit gemeld. De 280px staat nu in `@media (min-width: 769px)`, elkaar uitsluitend met de mobiele stand i.p.v. een cascade-gevecht (`css-layout.md` §4). Gemeten @375px: input 308→244, min-content 400→232, track 400→336, kaartrand **412→348 == containerrand**; @1280px input 316×48, knop 137, kaart 672 — identiek aan vóór. (b) **De teller loog tegen schermlezers op de skip-link:** `blog-filter.js` las élke hash als categorie, dus `/blog/#main-content` — het doel van de skip-link, de eerste bediening die een toetsenbordgebruiker tegenkomt — meldde "0 van 15 artikelen" in een `role="status"`-regio terwijl CSS alle 15 toonde. Idem `#newsletter` (dat id bestaat op de pagina). Valideert nu tegen de `.category-target`-ids die de pagina zélf declareert; bijvangst is dat `aria-current` bij een onbekende hash nu op "Alle posts" staat, wat de visuele stand al deed. **NEW 3 guards** in `blog-navigation.spec.js`, elk met zelfbewakende tak, plus de hardcoded `15` uit de bestaande tellertest gehaald (leidt het totaal nu uit de DOM af, zodat blogpost #16 hem niet omgooit). **Drie mutanten, drie verschillende asserties, elk 1 failed / 10 passed:** `order` weg → *geen filterstand begint met het nieuwsbriefblok*; vaste breedte terug → *geen grid-item steekt buiten de container*; hash-validatie terug → *een hash die geen categorie is*. **Twee eigen meetfouten die dit kostte, beide gevangen door A/B te meten i.p.v. te lezen:** (a) de eerste plaatsing van de `order`-regel stond vóór een tweede `width: 280px` verderop in hetzelfde bestand — gelijke specificiteit, dus die had gewonnen; het live-experiment zei "werkt" omdat een geïnjecteerde `<style>` altijd als laatste komt (`css-layout.md` §13). (b) de eerste versie van de nieuwsbrief-guard bleef **groen** op mutant 1: `page.goto` naar een URL die alleen in het fragment verschilt is een same-document navigatie, dus er herlaadde niets. Nu een unieke query per stand — ook representatiever, zo komt een bezoeker via een gedeelde link binnen. Verificatie: `blog-navigation` 3 motoren 32 passed / 1 flaky lokaal en **33 passed tegen productie**, `validate-docs --deep` 18/18, `validate-blogs` 16/16, aangrenzende blogspecs 34 passed. De lokale flaky is de bestaande test op regel 120 (firefox) en faalt op **navigeren** (wachten op `sibforms.com`), niet op een assertie — 3× los herhaald alle drie groen in ~7s. `blog.css?v=229`→`?v=230` (16 bestanden), `blog-filter.js?v=1`→`?v=2`.
77. [x] **De terminal rendeerde `[→]` in Chromium als `→]` ✅ OPGELOST Sessie 233 — de openingshaak schildert niet (Sessie 233)** — gevonden bij het her-capturen van de launch-visuals: het GIF-eindframe en beide PNG's tonen ` →] Typ 'next' voor je volgende stap`. **De DOM klopt** (`[<span class="marker-arrow">→</span>] Typ…`, codepoint U+005B aanwezig), de `[` wordt alleen niet geschilderd. **Zelfbewakend gemeten** (layout-rects vóór én ná de screenshot identiek, niet-lege populatie): in één en hetzelfde beeld, alle drie op x=60 — `[✓]` inkt **185**, `[TIP]` inkt **255**, `[→]` inkt **9 en 13** (= achtergrondniveau). **Trigger is `help`:** vóór `help` bestaat de markerregel nog niet; direct erna is de haak weg en hij blijft weg na scroll, geforceerde repaint én een volledige relayout via viewport-wissel — dus geen paint-invalidatie-artefact maar een blijvende toestand. **Chromium-only:** dezelfde structuur meet 255 in firefox én webkit. **Oorzaakketen (gemeten met fontTools):** `styles/fonts/jetbrainsmono-latin.woff2` (229 codepoints) mist U+2192 `→`, U+2190 `←` én U+2713 `✓`, en `jetbrains-mono-box-subset.woff2` dekt alleen U+2500-257F. Die drie markers vallen dus terug op een systeemfont met een andere baseline — precies waaróm `.marker-arrow`/`.inline-arrow` bestaan (`renderer.js:436-442`). Die spans knippen de regel in font-runs en laten `[` als **run van één teken** over, en dát is wat Chromium niet schildert. ⚠️ **Niet gerepareerd, bewust:** de fix is óf de font opnieuw subsetten mét die drie codepoints (raakt PRD §13 'geen build-stap', vraagt de upstream-bron) óf markerrendering herstructureren (`_formatText` + 3 CSS-klassen + elke markerregel). Beide vragen een eigen guard — een spec die per markersoort asserteert dát de haken inkt hebben, ná `help`, in Chromium — plus mutanten. Op een vermoeden repareren zou hier een derde CSS-hack op een fontprobleem stapelen. ⚠️ Mijn geïnjecteerde `<div>`-varianten gaven wisselende uitkomsten afhankelijk van hun positie in de output; die zijn **niet** als bewijs gebruikt. Het bewijs zijn de échte regels + de rauwe kolomscan van het artefact. **Het is een regressie:** de oude capture (vóór 6 jul, want hij draagt nog de `[?] TIP:`-marker van vóór `43eeb58`) toont de haak **wél**. Er is dus tussen begin juli en nu iets veranderd. **Eén lead al uitgesloten:** `font-variant-ligatures: none` uit Sessie 229 (`aab57a2`) is **niet** de trigger — terugzetten op `normal` én op `contextual` laat de inkt op 9 staan. **FIX (Sessie 233): de oorzaak in het brondocument, niet het symptoom.** De subset is opnieuw gebouwd uit de upstream JetBrains Mono (SIL OFL, licentie stond al in `styles/fonts/LICENSES/`): as beperkt tot wght 400–800 zoals het origineel, gesubset op de bestaande 229 codepoints **+ U+2190/U+2192/U+2713**, features gelijkgehouden op `calt,ccmp,frac,locl,mark` (met `--layout-features='*'` sleepte hij 36 features en 123 alternate-glyphs mee, +4,3 KB). U+00AD zat niet in upstream maar wél in de oude subset, dus expliciet teruggemapt op de gewone hyphen — anders was de wijziging niet strikt additief. **Geverifieerd vóór installatie:** 232 codepoints, niets kwijt, upem 1000, as 400–800, en **0 advance-width-verschillen over de 229 gedeelde codepoints** — dus geen layout-verschuiving. Bestand **31.432 → 30.272 bytes** (1.160 kleiner, mét drie glyphs erbij). Daarna kon de hack weg: `.marker-arrow` volledig uit `renderer.js` + `terminal.css`, en `.inline-arrow` behield kleur/marge maar verloor `position:relative;top:-.2em`. **Gemeten ná de fix** (lokaal, no-store server, drie motoren): haak schildert overal, pijl t.o.v. haak chromium +0,5px / firefox 0,0 / webkit +0,5 — béter dan de opgetilde span (−0,5) en ver beter dan de span weghalen zónder font-fix (+3,5px). De `←` in de nmap-output staat op 0,5px van zijn referentieletter op alle drie de regels. **Guard:** NEW `tests/e2e/marker-brackets.spec.js` (2 tests × 3 motoren) met drie asserties: populatie (faalt óók bij nul treffers), haak-schildert (pixels, niet DOM) en pijl-op-de-haaklijn. **3 mutanten, 3 verschillende asserties:** M1 span terug → structureel rood op 3 motoren + haak-inkt rood **alleen op chromium** (bewijst dat die tak motorspecifiek werkt); M2 unicode-range weer inperken → alleen uitlijning rood op alle 3; M3 marker hernoemen → populatie rood mét diagnostische melding. ⚠️ De guard betrapte eerst mijn **eigen** meetfout: op webkit is de screenshot in device-pixels en de rect in CSS-pixels, waardoor álle vier de markers als "geen inkt" lazen — inclusief `[?]` en `[✓]` die aantoonbaar renderen. Nu `devicePixelRatio`-bewust. **Regressie:** 251 passed / 0 failed / 4 skipped over `responsive-ascii-boxes` + `font-ligatures` + `marker-brackets`, drie motoren, mét eindblok. Bundel +480 bytes (alleen commentaar) → marge 14,68 KB; het font telt niet mee in `RUNTIME_SOURCE` maar scheelt de bezoeker wél 1,16 KB.
78. [x] **Launch-kit fire-ready gemaakt na 38 dagen stilstand ✅ Sessie 233** — aanleiding was Heisenberg's tegenvraag op mijn eerste antwoord: de site is 38 dagen live (launch-dag 29 jul), drie kanalen zijn nooit afgevuurd (EHGN-projectpost, Show HN, r/SideProject), `launch-checklist.md:25` noemt dat blok *"Morgen oppakken"* en dat "morgen" is 37 dagen geleden. Sessies 221-232 waren allemaal technisch. **Gedaan:** feitentabel hergeteld tegen de repo (**42** command-files, was 41; **14** blogposts, was 13 — beide vloeren `40+`/`12+` overleven de drift, en dát is precies waarom ze als vloer staan); twee stale instructies weg (de kop stuurde nog naar de op 22 jul overgeslagen demand-validatie #44, en de D-1-lijst stond onafgevinkt terwijl `launch-checklist.md` §Stand meldt dat hij op 29 jul is uitgevoerd — twee documenten die dezelfde toestand bijhielden); NEW **§6** met de drie open kanalen als definitieve plak-klare tekst, inclusief de HN-valkuil (URL-veld invullen → tekstveld leeg). **Gemeten:** alle bestemmingslinks + **28 sitemap-URL's op HTTP 200**, 0 falers. ⚠️ **Wat NIET is gedaan en van Heisenberg blijft:** het posten zelf (eigen accounts, en de checklist-regel "kanaal dat je niet kunt bewaken → niet posten" geldt voor mij dubbel) en de GA4-funnel + GSC Coverage (login). Zolang die data niet wordt opgehaald blokkeren #45 en #59 zichzelf.
...
**Mailmigratie TransIP → Zoho Mail (Sessie 234) ✅:**
1. [x] Zoho Mail Free op EU-datacenter Nederland; `contact@` + `dmarc@` als aliassen op één superuser
2. [x] MX → `mx.zoho.eu` 10/20/50, SPF-include vervangen mét behoud van Brevo, 2048-bit DKIM, DMARC-rua → `dmarc@`
3. [x] Beide verzendpaden geverifieerd met echte mail: Zoho SPF/DKIM/DMARC pass, Brevo DKIM-aligned pass
4. [x] TransIP E-mail Only opgezegd (~EUR 96/jaar → EUR 0); vijf verweesde mailrecords opgeruimd
5. [x] `.claude/plans/check-mail.sh` — 12 checks, bewaakt óók wat intact moest blijven
6. [x] `privacy.html`: Zoho in verwerkerstabel + §11; kop/footer-versiedrift van elf maanden rechtgetrokken
7. [ ] DMARC `p=none` → `p=quarantine` zodra de rapporten op `dmarc@` alleen eigen verzendpaden tonen
...
<!-- LET OP: validate-docs.sh Check 2 zoekt de footer-sessiemarker in de LAATSTE 30 REGELS, en elke
     **Versie:**-entry hieronder duwt hem een regel verder van het einde. Het aantal dat de lijst mag
     houden staat ONDER de lijst (de cursieve regel) — dat is de enige plek waar het hoort te staan.
     Hier stond tot Sessie 232 een tweede getal ("MAX 8"), achtergebleven toen de limiet bij Sessie 230
     naar 5 ging; in Sessie 232 stonden er daardoor 7. Een kopie van een regel verjaart, de eigenaar niet.
     Faalgeschiedenis: Sessie 224 (merk op 31) en Sessie 230 — beide keren door de groei van de lijst
     zelf, niet door de wijziging van die sessie. Oudere entries staan volledig in
     docs/sessions/current.md en de range-archieven. -->
**Laatst bijgewerkt:** 15 sep 2026 (Sessie 234; de mailbox kostte ~EUR 96/jaar, en de gratis standaardroute sterft in januari 2027)
**Versie:** 6.07 (Sessie 234 — **De mailbox kostte ~EUR 96/jaar en er was geen kleiner pakket.** Zoho Mail Free op het EU-datacenter in Nederland. De regio hangt af van de ingang, niet van je IP: via `zoho.com/nl` wijst de aanmeldknop naar `.com`, via `zoho.eu` naar `.eu` — en de keuze ligt onherroepelijk vast bij het aanmaken. Brevo slaagt op DKIM, niet op SPF: de Return-Path is Brevo's eigen bounce-domein, dus de SPF-include wordt voor die mail niet geraadpleegd — de `brevo*._domainkey`-CNAMEs doen het werk. `check-mail.sh` bewaakt beide kanten van de wissel; de eerste versie had een guard die niet kón falen. Historie: `docs/sessions/current.md`)
**Versie:** 6.06 (Sessie 233 — **De pijl ontbrak in de font-subset, en die ene hack liet de haak van `[→]` onzichtbaar.** `jetbrainsmono-latin.woff2` miste U+2192/U+2190/U+2713; die vielen terug op een systeemfont met een andere baseline, `renderer.js` compenseerde met een opgetilde span, en die liet `[` achter als tekst-run van één teken vóór een elementgrens — wat Chromium niet schildert. Gerepareerd in het brondocument: subset herbouwd uit upstream (232 codepoints, 0 advance-width-verschillen, 1.160 bytes kleiner), hack weg, NEW `marker-brackets.spec.js` met 3 mutanten op 3 asserties. Ook: de "cirkels" in de launch-GIF waren dithering van een echte vignette, geen site-bug — en de visuals bleken de take van vóór 6 juli. Historie: `docs/sessions/current.md`)
**Versie:** 6.05 (Sessie 232 — **De bloat zat niet in de code.** `src/` (118 modules), `styles/` (11) en `assets/` (36) bevatten nul verweesde bestanden en nul md5-duplicaten over 387 getrackte bestanden; wat overbleef waren artefacten die niemand meer las. **153M → 53M**, waarvan `.playwright-mcp/` 55 MB in 1111 screenshots over vijf maanden en `git gc` 3388 losse objecten naar één pack (69M → 26M). Twee "debugtests" konden niet falen op hun eigen onderwerp: alle zes `expect()`-calls in `debug-console`/`debug-storage` klikten enkel het legal-modal weg, terwijl de echte dekking assertief in `persistence-flush` en `vfs-versioning` staat. Grootste getrackte bestand was `lighthouse-m9-baseline.json` (637 KB, groter dan TASKS.md) waarnaar alleen een gitleaks-uitsluitingsregel verwees — en van de vier exclusies bleek er na meting maar één van dat bestand afhankelijk. NEW Check 20 (twee drempels: warn 10 MB, fail 50 MB; plus een tak die faalt zodra er iets getrackt onder die paden staat), 5 mutanten op 5 verschillende asserties. 42 specs / 315 decl; 1520 passed over drie motoren. Bundel 1104,85/1120 KB. ⚠️ Mijn eerste volle run gaf exit 0 met **862 did not run** — `tail`'s exit-code, niet die van Playwright, en mijn eigen global-timeout kapte de run af. Sessie 229 documenteerde dit patroon al; daarom staat het nu in `.claude/rules/meten-en-guards.md` i.p.v. in een derde sessielog. Historie: `docs/sessions/current.md`)
**Versie:** 6.04 (Sessie 231 — **`publish = "."` zette de bron van vier betaalde gidsen op de CDN.** Achteraf gereconstrueerd: deze sessie kreeg destijds geen `/summary`, terwijl 8 codebestanden zichzelf al "Sessie 231" noemen. Zonder build-stap is de publish-root de repo-root, dus alles wat git trackt werd geserveerd — de vier `.typ`-bronnen gaven **HTTP 200** op productie (24.584 / 36.373 / 31.067 / 17.830 bytes), naast een sessie-archief van 388 KB met drie privé-mailadressen. `Disallow: /docs/` is indexeringsadvies, geen toegangscontrole. Check 19 erbij met de **populatie omgedraaid** — élke getrackte top-level entry moet zich verantwoorden i.p.v. een blokkeerlijst, want een lijst-guard bewaakt zijn lijst en juist daardoor kon `docs/products/` meeliften — en hij betrapte in zijn eerste CI-run de commit die hem introduceerde. Verder: 450 KB Brevo-`main.js` weg na ablatiemeting (render byte-identiek, zelfde MD5) omdat `brevo-submit.js` het submit-event al in de capture-fase onderschept; `sib-styles.css` blijft (74px verschuiving) en staat nu in het privacybeleid; en drie localStorage-lezers die `"hoi"`, `[]` en `null` als object indexeerden — geldige JSON van de verkeerde vorm komt nooit in een `catch`. Historie: `docs/sessions/current.md`)
**Versie:** 6.03 (Sessie 230 — **Het nieuwsbriefblok viel buiten de filterpopulatie, en rekte via één grid-track alle 15 kaarten op.** Het `:target`-filter verbergt uitsluitend `.blog-post-card`; het nieuwsbriefblok is er geen, dus het bleef in élke stand op DOM-positie 4 staan — gemeten 4 van de 6 categorieën met het formulier bovenaan i.p.v. een artikel. Opgelost met een klasse-gebaseerde `order: 1` op `.category-target`, zodat groep 4 zichzelf bijhoudt terwijl groep 1 t/m 3 moeten enumereren. Twee stille defecten in hetzelfde blok: een vaste `width: 280px` die de mobiele `width: 100%` op specificiteit versloeg gaf het blok een min-content van 400px — één impliciete `auto`-track betekent dat het breedste item de rest sleept, dus alle 15 kaarten renderden 400px in een container van 336px, onzichtbaar weggeknipt door `overflow-x: hidden`; en `blog-filter.js` las élke hash als categorie, waardoor de skip-link `#main-content` "0 van 15 artikelen" liet omroepen in een `role="status"`-regio. Twee eigen meetfouten onderweg, allebei van de vorm 'afwezigheid van een faalsignaal is geen bewijs': een geïnjecteerde `<style>` won alleen op bronvolgorde en bewees niets over de cascade, en een guard bleef groen op een echte regressie omdat `goto` naar een fragment-only URL niet herlaadt. Historie: `docs/sessions/current.md`)
...
_De versiestapel houdt de **nieuwste 5**. Elke regel erbij duwt de `Laatst bijgewerkt`-marker een regel verder van het bestandseinde, en Check 2 zoekt hem in de laatste 30 — bij Sessie 230 brak dat. Oudere versienoten staan volledig in `docs/sessions/current.md` en de range-archieven._

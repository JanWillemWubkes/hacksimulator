...
**Laatst bijgewerkt:** 14 aug 2026 (Sessie 222; box-randen braken verticaal — vier eerdere fixes zochten in de breedte)
**Sprint:** Sessie 222: **De box-randen braken verticaal — en vier eerdere fixes zochten allemaal in de breedte** (`260f8af`, gepusht). Opdracht: drie foto's van `metasploit`, `next` en `man metasploit` waarop de omlijning rond de terminal-output stukgaat, met de vraag hoe dat kan na zoveel pogingen. **De breedte-kant klopte al**: het inline boxfont is byte-identiek aan de schijf-subset en `loaded`, alle box-glyphs meten exact **10,8px** net als latin (beide woff2's 1000 upm / advance 600), en de rechterrand-spreiding is **≤0,04px** over acht box-commando's @1440/900/620px — nul wraps, nul overflow. De Sessie 204/205-machinerie doet dus gewoon zijn werk. De breuk zit in de **verticale** as, die nooit onderzocht was: `.terminal-line` draagt `margin-bottom: 4px` en een verticale glyph (│/┃) tekent alléén binnen zijn eigen linebox. Gemeten op de gerenderde randkolom: **12 stukjes van 27px met 4px gaten**, en **8px** op regels met een pijl, want `vertical-align: .2em` op `.marker-arrow` rekt de linebox 3,59px op. Alleen de marge weghalen is **niet genoeg** (variant A liet pitch {27; 30,59} staan); pas marge weg **plus** de pijl via `position: relative` gaf uniform **{27}**. Tweede, kleinere breuk bij het versmallen van het venster: onder 768px wordt de regelafstand **25,6px** (fractioneel) tegen een glyphhoogte van ~25,78px — te weinig overlap, dus de rasterisatie liet naden van 1px vallen (**97,8% dekking, 9 naden**). Box-regels volgen nu `--line-height` (1,5 → integer op 18px én 16px). Resultaat: **0 gaten** over acht commando's, rand is **één run van 293px** i.p.v. 12 stukjes, **100,0% dekking** @760px na resize, rechteruitlijning onveranderd. NEW verticale-continuïteitsdetector in `responsive-ascii-boxes.spec.js` (pitch ≤ ink-hoogte van de randglyph via canvas — één predicaat dat marge, `vertical-align` én een te grote `line-height` dekt), plus `next`/`metasploit` die **niet in COMMANDS stonden** terwijl het juist de gemelde commando's waren, en een reflow-test naar 700px. Drie mutanten falen **verschillend**: marge terug 9 rood, `vertical-align` terug 7 rood (metasploit blijft groen — geen pijl), line-height terug 1 rood (alleen <768px). Box-spec **152 passed** over chromium/firefox/webkit, `validate-docs.sh` exit 0, bundel **1102,37/1120 KB** (marge 1,57%).

...
**Laatst bijgewerkt:** 09 aug 2026 (Sessie 217 — GEEN architectuur-wijziging: vier pre-existing punten opgeruimd binnen de bestaande static-site-architectuur. De CTA-reserve verhuist van `body` naar `.landing-footer` (CSS-only, `:has()` op elementaanwezigheid), de eyebrow-badge krijgt een `--eyebrow-text`-token in het bestaande theme-aware tokenpatroon, en het terminal-overflow-item was al gefixt en krijgt alleen de ontbrekende regressietest. Geen nieuwe afhankelijkheid, geen build-stap, geen tech/design/security/deployment-shift, ownership ongewijzigd. **Budget-noot:** de verhoging 1100 → 1120 KB betreft opnieuw het E2E-*driftalarm* over ongeminificeerde broncode (woont in TASKS.md), niet het Terminal-App-Core-budget van <400 KB — geen van de drie wijzigingen zit in de module-graph van `terminal.html`.)

...
**Laatst bijgewerkt:** 5 sep 2026 (Sessie 232 — **geen architectuurwijziging, wél een ownership-grens die eerder nergens stond:** artefacten die buiten git om groeien hebben geen eigenaar en dus geen guard. `.gitignore` maakt ze onzichtbaar in `git status`, de bundel-budgetten in TASKS.md tellen ze niet mee, en een schone werkboom zegt niets over hun omvang — `.playwright-mcp/` kon daardoor ongemerkt naar 55 MB groeien. Belegd bij `validate-docs.sh` Check 20 en de opruimstap in `/summary` Step 7; Sessie 231 is in dezelfde ronde achteraf gereconstrueerd, want die kreeg destijds geen `/summary` terwijl 8 codebestanden zichzelf al zo noemen.)

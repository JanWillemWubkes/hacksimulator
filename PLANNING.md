...
**Laatst bijgewerkt:** 02 aug 2026 (Sessie 207 — geen architectuur-wijziging; audit van oude follow-up-lijstjes + 4 werkpakketten: CSS-tokenfix (`--color-border` → `--color-border-dark-frame` op navbar/footer) + style-guide-correcties, een vijfde transitie-object in `next.js`, een download-export naast de bestaande klembord-export in `tutorial/certificate.js`, en NEW `scripts/build-blog-og-images.mjs` — alles binnen de bestaande static-site- + terminal-renderer-architectuur; het build-script is tooling zoals `build-product-covers.mjs`/`capture-launch-visuals.mjs`, geen tech/design/security/deployment-shift, PLANNING.md ownership ongewijzigd. Regel meegenomen voor sessie-counter-alignment)

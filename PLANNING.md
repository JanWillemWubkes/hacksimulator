...
**Laatst bijgewerkt:** 09 aug 2026 (Sessie 218 — GEEN architectuur-wijziging: de educatiestrook onder de terminal is gehalveerd en klikbaar gemaakt binnen de bestaande static-site-architectuur. De nieuwe `src/ui/edu-visibility-tracking.js` volgt het bestaande single-purpose-trackingmodulepatroon (`cta-tracking.js`, `newsletter-tracking.js`) en hangt alleen aan `terminal.html`; `edu_section_reached` is toegevoegd aan de bestaande `src/analytics/events.js`-catalogus. Geen nieuwe afhankelijkheid, geen build-stap, geen tech/design/security/deployment-shift, ownership ongewijzigd. **Budget-noot:** deze sessie voegde ~3,3 KB toe (1091,02 van 1120 KB) — dat is opnieuw het E2E-*driftalarm* over ongeminificeerde broncode, niet het Terminal-App-Core-budget van <400 KB; `terminal-education.css` en de nieuwe module zitten wél op `terminal.html`, maar de CSS stond daar al en de module is 1,7 KB.)

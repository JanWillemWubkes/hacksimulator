...
**Laatst bijgewerkt:** 09 aug 2026 (Sessie 219 — **design-system-uitbreiding, geen architectuur-wijziging.** §Design System → Layout Principes #5 legt het sectieritme vast: *pagina = oppervlak, band = verdieping, kaart = verhoging*, met het nieuwe token `--color-bg-alt` en de modifier `.section-band`. Dat is een regel, geen waarde: hij verklaart waarom een band ónder `--color-bg` moet (de kaarten zijn tinten van `#161b22`, dus een lichtere band maakt hun verschil nul) en zet een norm op de langste reeks zonder achtergrondwissel (~1,4 viewport). Vervangt drie ad-hoc rgba-waarden waarvan er één in light mode aantoonbaar Δ0 was. Geen nieuwe afhankelijkheid, geen build-stap, geen tech/security/deployment-shift, ownership ongewijzigd. **Budget-noot:** +2,88 KB (1093,90 van 1120 KB, marge 2,3%) terwijl de diff nétto CSS-regels verwijdert — het commentaar dat beide rekensommen vastlegt is langer dan de code. Terminal Core wordt met nul bytes geraakt: `landing.css` zit niet in de module-graph van `terminal.html`.)

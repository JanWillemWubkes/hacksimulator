...
**Laatst bijgewerkt:** 20 aug 2026 (Sessie 229 — **geen architectuurwijziging, wél een sitebrede typografie-constraint.** JetBrains Mono ligeert via `calt` (standaard aan), waardoor de terminal `>=` als ≥ en `$pdo->prepare` als `$pdo→prepare` toonde. `font-variant-ligatures: none` staat nu op `*` — omgedraaide populatie i.p.v. een lijst mono-selectors, met gemeten sub-pixel-kosten in prose. Zie `docs/style-guide.md` §Typography System en `.claude/rules/css-layout.md` §20.)
